...
pub fn expand_grid<'a>(
    ...
    for (dieid, grid) in grids {
        let (_, mut die) = egrid.add_die(grid.columns.len(), grid.regs * 48);
        ...
        let xlut: EntityVec<ColId, usize> = die
            .cols()
            .map(|x| {
                if x < grid.col_cfrm {
                    x.to_idx()
        ...
        for (col, &cd) in &grid.columns {
            if disabled.contains(&DisabledPart::Column(dieid, col)) {
                continue;
            }
            let x = xlut[col];
            let cle_x_bump_cur;
            ...
            for row in die.rows() {
                ...
                if matches!(cd.r, ColumnKind::Cle | ColumnKind::CleLaguna) {
                    ...
                    if has_bli_r {
                        die.fill_term_pair_anon((col, row), (col + 1, row), cle_bli_e, cle_bli_w);
                    } else {
