use super::{GeomDb, WireClass, WireConn, PortClass, PortConn, TileClass, TieState, Grid, Tile, Port};
...
pub trait GeomDbBuilder {
    ...
    fn make_hbus_wire(&mut self, name: &str, cls: &str, bus: usize, has_multicell_drive: bool) -> usize;
    fn make_vbus_wire(&mut self, name: &str, cls: &str, bus: usize, has_multicell_drive: bool) -> usize;
    fn make_tie_wire(&mut self, name: &str, cls: &str, state: TieState) -> usize;
    fn make_simple_pconn(&mut self, wire_down: usize, wire_up: usize, pcls_down: usize, pcls_up: usize) -> usize;
}
...
impl GeomDbBuilder for GeomDb {
    ...
    fn make_simple_pconn(&mut self, wire_down: usize, wire_up: usize, pcls_down: usize, pcls_up: usize) -> usize {
        ...
        match &mut wd.conn {
            WireConn::Internal => wd.conn = WireConn::Port{up: Some((psu, cu)), down: Vec::new()},
            WireConn::Port{up, ..} => {
                assert!(up.is_none());
                *up = Some((psu, cu));
            },
        ...
        match &mut wu.conn {
            WireConn::Internal => wu.conn = WireConn::Port{up: None, down: vec![(psd, cd)]},
            WireConn::Port{down, ..} => {
                down.push((psd, cd));
            },
            _ => panic!("simple_pconn on bus wire"),
...
pub trait GridBuilder {
    ...
    fn fill_port_pair(&mut self, geomb: &GeomDb, xya: (usize, usize), xyb: (usize, usize), pc: (usize, usize));
...
impl GridBuilder for Grid {
    ...
    fn fill_port_pair(&mut self, geomdb: &GeomDb, xya: (usize, usize), xyb: (usize, usize), pc: (usize, usize)) {
