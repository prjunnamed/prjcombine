    ...
    for (tcrd, tile) in egrid.tiles() {
        ...
        match &kind[..] {
            ...
            "HCLK" => {
                ...
                nnode.add_bel(bels::GLOBALSIG, format!("GLOBALSIG_X{x}Y{y}", y = y / 40));
            }
            "HCLK_QBUF" => {
                namer.ngrid.name_tile(
                    tcrd,
            ...
            "DSP" => {
                ...
                nnode.add_bel(bels::TIEOFF_DSP, format!("TIEOFF_X{tx}Y{y}"));
            }
            "IO" => {
            ...
            "HCLK_IOI" => {
                ...
                nnode.add_bel(bels::BUFIO0, format!("BUFIODQS_X{iox}Y{y}", y = hy * 4 + 2));
                nnode.add_bel(bels::BUFIO1, format!("BUFIODQS_X{iox}Y{y}", y = hy * 4 + 3));
                nnode.add_bel(bels::BUFIO2, format!("BUFIODQS_X{iox}Y{y}", y = hy * 4));
                nnode.add_bel(bels::BUFIO3, format!("BUFIODQS_X{iox}Y{y}", y = hy * 4 + 1));
                nnode.add_bel(bels::BUFR0, format!("BUFR_X{iox}Y{y}", y = hy * 2 + 1));
                nnode.add_bel(bels::BUFR1, format!("BUFR_X{iox}Y{y}", y = hy * 2));
                nnode.add_bel(bels::BUFO0, format!("BUFO_X{iox}Y{y}", y = hy * 2 + 1));
                nnode.add_bel(bels::BUFO1, format!("BUFO_X{iox}Y{y}", y = hy * 2));
                nnode.add_bel(bels::IDELAYCTRL, format!("IDELAYCTRL_X{iox}Y{hy}"));
                nnode.add_bel(bels::DCI, format!("DCI_X{iox}Y{hy}"));
            }
            "CMT" => {
                ...
                for (i, slots) in [bels::BUFHCE_W, bels::BUFHCE_E].into_iter().enumerate() {
                    for j in 0..12 {
                        nnode.add_bel(slots[j], format!("BUFHCE_X{i}Y{y}", y = by * 12 + j));
                    }
                }
                nnode.add_bel(bels::MMCM0, format!("MMCM_ADV_X{bx}Y{y}", y = by * 2));
                nnode.add_bel(bels::MMCM1, format!("MMCM_ADV_X{bx}Y{y}", y = by * 2 + 1));
                nnode.add_bel(bels::PPR_FRAME, format!("PPR_FRAME_X{bx}Y{by}"));
            }
            "PMVIOB" => {
                let naming = if row < chip.row_bufg() {
                    "CMT_PMVA_BELOW"
            ...
            "CFG" => {
                ...
                nnode.add_bel(bels::ICAP0, "ICAP_X0Y0".to_string());
                nnode.add_bel(bels::ICAP1, "ICAP_X0Y1".to_string());
                nnode.add_bel(bels::PMV0, "PMV_X0Y0".to_string());
                nnode.add_bel(bels::PMV1, "PMV_X0Y1".to_string());
                nnode.add_bel(bels::STARTUP, "STARTUP_X0Y0".to_string());
                nnode.add_bel(bels::CAPTURE, "CAPTURE_X0Y0".to_string());
                nnode.add_bel(bels::FRAME_ECC, "FRAME_ECC".to_string());
                nnode.add_bel(bels::EFUSE_USR, "EFUSE_USR_X0Y0".to_string());
                nnode.add_bel(bels::USR_ACCESS, "USR_ACCESS_X0Y0".to_string());
                nnode.add_bel(bels::DNA_PORT, "DNA_PORT_X0Y0".to_string());
                nnode.add_bel(bels::DCIRESET, "DCIRESET_X0Y0".to_string());
                nnode.add_bel(bels::CFG_IO_ACCESS, "CFG_IO_ACCESS_X0Y0".to_string());
            ...
            "GTX" => {
                ...
                for i in 0..4 {
                    ...
                    nnode.add_bel(bels::GTX[i], format!("GTXE1_X{gx}Y{gy}", gy = gy * 4 + i));
                }
                nnode.add_bel(
                    bels::IPAD_CLKP0,
                ...
                nnode.add_bel(bels::BUFDS0, format!("IBUFDS_GTXE1_X{gx}Y{y}", y = gy * 2));
                nnode.add_bel(
                    bels::BUFDS1,
            ...
            "GTH" => {
                ...
                nnode.add_bel(bels::GTH_QUAD, format!("GTHE1_QUAD_X{gx}Y{gthy}"));
                nnode.add_bel(
                    bels::BUFDS0,
