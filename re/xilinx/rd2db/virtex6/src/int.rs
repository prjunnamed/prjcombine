...
pub fn make_int_db(rd: &Part) -> (IntDb, NamingDb) {
    ...
    if let Some(&xy) = rd.tiles_by_kind_name("EMAC").iter().next() {
        let mut int_xy = Vec::new();
        let mut intf_xy = Vec::new();
        let n = builder.ndb.get_node_naming("INTF.EMAC");
        for dy in 0..10 {
            int_xy.push(Coord {
                x: xy.x - 2,
                y: xy.y + dy,
    ...
    if let Some(&xy) = rd.tiles_by_kind_name("PCIE").iter().next() {
        let mut int_xy = Vec::new();
        let mut intf_xy = Vec::new();
        let nl = builder.ndb.get_node_naming("INTF.PCIE_L");
        let nr = builder.ndb.get_node_naming("INTF.PCIE_R");
        for dy in 0..20 {
            int_xy.push(Coord {
                x: xy.x - 4,
                y: xy.y - 10 + dy,
    ...
    for (tkn, nn) in [("HCLK_GTX", "GTX"), ("HCLK_GTX_LEFT", "GTX_LEFT")] {
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            for i in 0..4 {
                bels.extend([
                    builder
                        .bel_xy(format!("IPAD.RXP{i}"), "IPAD", 0, 1)
                        .raw_tile(i + 1)
                        .pins_name_only(&["O"]),
                    builder
                        .bel_xy(format!("IPAD.RXN{i}"), "IPAD", 0, 0)
                        .raw_tile(i + 1)
                        .pins_name_only(&["O"]),
                    builder
                        .bel_xy(format!("OPAD.TXP{i}"), "OPAD", 0, 1)
                        .raw_tile(i + 1)
                        .pins_name_only(&["I"]),
                    builder
                        .bel_xy(format!("OPAD.TXN{i}"), "OPAD", 0, 0)
                        .raw_tile(i + 1)
                        .pins_name_only(&["I"]),
                ]);
            }
