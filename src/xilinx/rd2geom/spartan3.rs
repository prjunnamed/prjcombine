...
use super::cfg::{GeomBuilderConfig, IntTileInfo, IntTermInfo, IntPassInfo, IntBufInfo, IntDoubleBufInfo, TileInfo, TieSiteInfo, TileAnchor};
...
impl Spartan3GeomMaker {
    pub fn new(fam: &str) -> Self {
        ...
        if family == Family::S3 {
            ...
            tiles.append(&mut vec![
                TileInfo::site_int("SITE_CLB", &["CENTER_SMALL", "CENTER_SMALL_BRK"]),
            ]);
            if family == Family::S3ADSP {
                tiles.append(&mut vec![
                    TileInfo::site_rect("SITE_BRAM", &["BRAMSITE2_3M", "BRAMSITE2_3M_BRK", "BRAMSITE2_3M_BOT", "BRAMSITE2_3M_TOP"], (1, 4), (-1, 0)),
                    TileInfo::site_vert_r("SITE_DSP", &["MACCSITE2", "MACCSITE2_BRK", "MACCSITE2_BOT", "MACCSITE2_TOP"], (4, 0)),
                ]);
            } else {
                tiles.append(&mut vec![
                    TileInfo::site_rect("SITE_BRAM", &["BRAMSITE2", "BRAMSITE2_BRK", "BRAMSITE2_BOT", "BRAMSITE2_TOP"], (1, 4), (-1, 0)),
                ]);
            }
        ...
        if family == Family::S3E {
            int_dbufs.append(&mut vec![
                IntDoubleBufInfo::v("COB_TERM_B", "COB_TERM_T", "INT_COB"),
            ]);
        }
    ...
    fn setup_int_omux(&mut self) {
        for (i, dirs) in [
            ...
            if i == 2 {
                self.builder.make_int_wire_cont("INT.OMUX2.S", "INT.OMUX", &["SE_S"], Dir::S, base);
            }
            if i == 9 {
                self.builder.make_int_wire_cont("INT.OMUX9.N", "INT.OMUX", &["NW_N"], Dir::N, base);
            }
        }
    }
    ...
    fn setup_int_imux(&mut self) {
        ...
        for i in 0..8 {
            self.builder.make_int_wire(&format!("INT.IMUX.IOCLK{}", i), "INT.IMUX.IOCLK", &[
                &format!("IOIS_CLK{}", i),
            ]);
        }
        ...
        for i in 0..4 {
            self.builder.make_int_wire(&format!("INT.IMUX.FAN.BX{}", i), "INT.IMUX.FAN", &[
                &format!("BX{}", i),
                &format!("IOIS_FAN_BX{}", i),
                &format!("CNR_BX{}", i),
                &if self.family == Family::S3ADSP {format!("BRAM_BX_B{}", i)} else {format!("BRAM_FAN_BX{}", i)},
                &format!("MACC_BX_B{}", i),
                &format!("BRAM_IOIS_FAN_BX{}", i),
                &format!("DCM_FAN_BX{}", i),
            ]);
            if self.family == Family::S3ADSP {
                self.builder.make_int_wire(&format!("INT.IMUX.FAN.BX{}.BOUNCE", i), "INT.IMUX.FAN", &[
                    &format!("BRAM_FAN_BX{}", i),
                    &format!("MACC_FAN_BX{}", i),
                ]);
            }
        }
        for i in 0..4 {
            self.builder.make_int_wire(&format!("INT.IMUX.FAN.BY{}", i), "INT.IMUX.FAN", &[
                &format!("BY{}", i),
                &format!("IOIS_FAN_BY{}", i),
                &format!("CNR_BY{}", i),
                &if self.family == Family::S3ADSP {format!("BRAM_BY_B{}", i)} else {format!("BRAM_FAN_BY{}", i)},
                &format!("MACC_BY_B{}", i),
                &format!("BRAM_IOIS_FAN_BY{}", i),
                &format!("DCM_FAN_BY{}", i),
            ]);
            if self.family == Family::S3ADSP {
                self.builder.make_int_wire(&format!("INT.IMUX.FAN.BY{}.BOUNCE", i), "INT.IMUX.FAN", &[
                    &format!("BRAM_FAN_BY{}", i),
                    &format!("MACC_FAN_BY{}", i),
                ]);
            }
        }

        for i in 0..32 {
            self.builder.make_int_wire(&format!("INT.IMUX.DATA{}", i), "INT.IMUX.DATA", &[
                &format!("{}{}_B{}", ["F", "G"][i >> 4], (i >> 2 & 3) + 1, i & 3),
                &format!("IOIS_{}{}_B{}", ["F", "G"][i >> 4], (i >> 2 & 3) + 1, i & 3),
                &format!("IOIS_STUB_{}{}_B{}", ["F", "G"][i >> 4], (i >> 2 & 3) + 1, i & 3),
                &format!("TBIOIS_{}{}_B{}", ["F", "G"][i >> 4], (i >> 2 & 3) + 1, i & 3),
                &format!("LRIOIS_{}{}_B{}", ["F", "G"][i >> 4], (i >> 2 & 3) + 1, i & 3),
                &format!("CNR_DATA_IN{}", i),
                [
                    "BRAM_DIA_B18", "BRAM_MULTINA_B15", "BRAM_MULTINB_B17", "BRAM_DIA_B1",
                    "BRAM_ADDRB_B0", "BRAM_DIB_B19", "BRAM_DIB_B0", "BRAM_ADDRA_B3",
                    "BRAM_DIA_B19", "BRAM_DIPB_B", "BRAM_MULTINA_B17", "BRAM_DIA_B0",
                    "BRAM_ADDRB_B1", "BRAM_DIB_B18", "BRAM_DIB_B1", "BRAM_ADDRA_B2",
                    "BRAM_DIA_B2", "BRAM_MULTINA_B14", "BRAM_MULTINB_B16", "BRAM_DIA_B17",
                    "BRAM_ADDRA_B0", "BRAM_DIB_B3", "BRAM_DIB_B16", "BRAM_ADDRB_B3",
                    "BRAM_DIA_B3", "BRAM_DIPA_B", "BRAM_MULTINA_B16", "BRAM_DIA_B16",
                    "BRAM_ADDRA_B1", "BRAM_DIB_B2", "BRAM_DIB_B17", "BRAM_ADDRB_B2",
                ][i],
                // 3A DSP version
                [
                    "", "BRAM_MULTINA_B1", "", "",
                ...
                [
                    "MACC_DIA_B18", "MACC_MULTINA_B1", "MACC_MULTINB_B17", "MACC_DIA_B1",
                    "MACC_ADDRB_B0", "MACC_DIB_B19", "MACC_DIB_B0", "MACC_ADDRA_B3",
                    "MACC_DIA_B19", "MACC_DIPB_B", "MACC_MULTINA_B3", "MACC_DIA_B0",
                    "MACC_ADDRB_B1", "MACC_DIB_B18", "MACC_DIB_B1", "MACC_ADDRA_B2",
                    "MACC_DIA_B2", "MACC_MULTINA_B0", "MACC_MULTINB_B16", "MACC_DIA_B17",
                    "MACC_ADDRA_B0", "MACC_DIB_B3", "MACC_DIB_B16", "MACC_ADDRB_B3",
                    "MACC_DIA_B3", "MACC_DIPA_B", "MACC_MULTINA_B2", "MACC_DIA_B16",
                    "MACC_ADDRA_B1", "MACC_DIB_B2", "MACC_DIB_B17", "MACC_ADDRB_B2",
                ][i],
                &format!("BRAM_IOIS_{}{}_B{}", ["F", "G"][i >> 4], (i >> 2 & 3) + 1, i & 3),
                &format!("DCM_{}{}_B{}", ["F", "G"][i >> 4], (i >> 2 & 3) + 1, i & 3),
                [
                    "", "", "DCM_CTLSEL0_STUB", "DCM_CTLSEL1_STUB",
                    "", "", "", "",
                    "", "", "", "",
                    "", "", "", "",
                    "DCM_DSSEN_STUB", "DCM_PSEN_STUB", "DCM_PSINCDEC_STUB", "DCM_RST_STUB",
                    "DCM_STSADRS1_STUB", "DCM_STSADRS2_STUB", "DCM_STSADRS3_STUB", "DCM_STSADRS4_STUB",
                    "DCM_CTLMODE_STUB", "DCM_FREEZEDLL_STUB", "DCM_FREEZEDFS_STUB", "DCM_STSADRS0_STUB",
                    "DCM_CTLSEL2_STUB", "DCM_CTLOSC2_STUB", "DCM_CTLOSC1_STUB", "DCM_CTLG0_STUB",
                ][i],
            ]);
        }
    }
    ...
    fn setup_int_out(&mut self) {
        for i in 0..8 {
            self.builder.make_int_out_wire(&format!("INT.OUT.FAN{}", i), "INT.OUT.FAN", &[
                // In CLBs, used for combinatorial outputs.
                ["X0", "X1", "X2", "X3", "Y0", "Y1", "Y2", "Y3"][i],
                ["IOIS_X0", "IOIS_X1", "IOIS_X2", "IOIS_X3", "IOIS_Y0", "IOIS_Y1", "IOIS_Y2", "IOIS_Y3"][i],
        ...
        for i in 0..16 {
            self.builder.make_int_out_wire(&format!("INT.OUT.SEC{}", i), "INT.OUT.SEC", &[
                ...
                [
                    "", "", "", "",
                    "BRAM_DOPA", "BRAM_DOPB", "", "BRAM_MOUT32",
                    "BRAM_MOUT7", "BRAM_MOUT6", "BRAM_MOUT5", "BRAM_MOUT4",
                    "BRAM_MOUT3", "BRAM_MOUT2", "BRAM_MOUT1", "BRAM_MOUT0",
                ][i],
                [
                    "", "", "", "",
                    "MACC_DOPA", "MACC_DOPB", "", "MACC_MOUT32",
                    "MACC_MOUT7", "MACC_MOUT6", "MACC_MOUT5", "MACC_MOUT4",
                    "MACC_MOUT3", "MACC_MOUT2", "MACC_MOUT1", "MACC_MOUT0",
                ][i],
                [
                    "BRAM_IOIS_PSDONE", "BRAM_IOIS_CONCUR", "BRAM_IOIS_LOCKED", "BRAM_IOIS_CLKDV",
                    "BRAM_IOIS_STATUS4", "BRAM_IOIS_STATUS5", "BRAM_IOIS_STATUS6", "BRAM_IOIS_STATUS7",
                    "BRAM_IOIS_STATUS0", "BRAM_IOIS_STATUS1", "BRAM_IOIS_STATUS2", "BRAM_IOIS_STATUS3",
                    "BRAM_IOIS_PTE2OMUX0", "BRAM_IOIS_PTE2OMUX1", "BRAM_IOIS_PTE2OMUX2", "BRAM_IOIS_PTE2OMUX3",
                ][i],
                [
                    "DCM_PSDONE", "DCM_CONCUR", "DCM_LOCKED", "DCM_CLKDV",
                    "DCM_STATUS4", "DCM_STATUS5", "DCM_STATUS6", "DCM_STATUS7",
                    "DCM_STATUS0", "DCM_STATUS1", "DCM_STATUS2", "DCM_STATUS3",
                    "DCM_PTE2OMUX0", "DCM_PTE2OMUX1", "DCM_PTE2OMUX2", "DCM_PTE2OMUX3",
                ][i],
                [
                    "", "", "", "",
                    "", "", "", "",
                    "", "", "", "",
                    "DCM_PTE2OMUX0_STUB", "DCM_PTE2OMUX1_STUB", "DCM_PTE2OMUX2_STUB", "DCM_PTE2OMUX3_STUB",
                ][i],
    ...
    fn setup_buses_dcm(&mut self) {
        for i in 0..4 {
            let _w = self.builder.make_int_wire(&format!("DCM.CLKPAD{}", i), "DCM.CLKPAD", &[
    ...
    fn fill_grid_cols(&self, part: &mut PartBuilder) {
        let bram_cols = part.find_anchor_gx_set(&TileAnchor::int(&["BRAM0", "BRAM0_SMALL", "BRAM0_SMALL_BOT"]));
