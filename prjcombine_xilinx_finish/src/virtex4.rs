...
pub fn finish(geom: GeomDb, tiledb: TileDb) -> Database {
    ...
    for dev in &geom.devices {
        let grids = dev.grids.map_values(|&grid| {
            let prjcombine_xilinx_geom::Grid::Virtex4(ref grid) = geom.grids[grid] else {
                unreachable!()
            };
            grid
        });
        let interposer = match geom.interposers[dev.interposer] {
            prjcombine_xilinx_geom::Interposer::None => None,
            prjcombine_xilinx_geom::Interposer::Virtex4(ref interposer) => Some(interposer),
            _ => unreachable!(),
        };
        let disabled: BTreeSet<_> = dev
            ...
            .iter()
            .map(|&dis| {
                let prjcombine_xilinx_geom::DisabledPart::Virtex4(dis) = dis else {
                    unreachable!()
                };
                dis
            })
        ...
        for devbond in dev.bonds.values() {
            let prjcombine_xilinx_geom::Bond::Virtex4(ref bond) = geom.bonds[devbond.bond] else {
                unreachable!()
            };
