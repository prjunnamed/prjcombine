...
pub fn make_int_db(rd: &Part) -> (IntDb, NamingDb) {
    ...
    // The long wires.
    let mid = builder.wire("LH.9", WireKind::MultiRoot, &["LH9"]);
    let mut prev = mid;
    ...
    let mid = builder.wire("LV.9", WireKind::MultiRoot, &["LV9"]);
    let mut prev = mid;
    ...
    for tkn in ["LIOB", "LIOB_MON", "CIOB", "RIOB"] {
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xtile(tslots::BEL, "IO", tkn, ioi_xy)
                .raw_tile(xy)
                .ref_int(int_xy, 0)
    ...
    for tkn in ["CMT_BOT", "CMT_TOP"] {
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            bels.push(bel);
            let mut xn = builder.xtile(tslots::BEL, "CMT", tkn, xy).num_tiles(10);
            for i in 0..10 {
    ...
    for (tkn, kind, has_bufr, has_io_s, has_io_n, has_rclk) in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            let mut xn = builder.xtile(tslots::HCLK_BEL, kind, kind, xy);
            let mut t = 0;
            if has_io_s {
                xn = xn
    ...
    for tkn in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xtile(tslots::HCLK_CMT, "HCLK_CMT", "HCLK_CMT", xy)
                .num_tiles(0)
                .raw_tile(xy.delta(1, 0))
                .bel(bel_hclk)
                .bel(bel_giob)
    ...
    for (tkn, tcname) in [
        ...
        let bt = if tcname == "CLK_CMT_B" { 'B' } else { 'T' };
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xtile(tslots::CLK, tcname, tcname, xy)
                .num_tiles(0)
                .bel(bel)
                .extract();
        }
    ...
    for (tkn, tcname) in [
        ...
        let bt = if tcname == "CLK_MGT_B" { 'B' } else { 'T' };
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
    ...
    for tkn in ["HCLK_BRAM_MGT", "HCLK_BRAM_MGT_LEFT"] {
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xtile(tslots::CLK, "HCLK_BRAM_MGT", "HCLK_BRAM_MGT", xy)
                .num_tiles(0)
                .bel(bel)
                .extract();
        }
    ...
    for (tkn, kind) in [("GT3", "GTP"), ("GTX", "GTX"), ("GTX_LEFT", "GTX")] {
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            let mut xn = builder.xtile(tslots::BEL, kind, tkn, xy).num_tiles(20);
            for i in 0..10 {
