...
use crate::{
    ...
    fuzz::FuzzCtx,
    fuzz_enum, fuzz_multi_attr_dec, fuzz_multi_attr_hex, fuzz_one_extras,
};
...
pub fn add_fuzzers<'a>(session: &mut Session<IseBackend<'a>>, backend: &IseBackend<'a>) {
    let Some(ctx) = FuzzCtx::try_new(session, backend, "PCIE", "PCIE", TileBits::Pcie) else {
        return;
    };
    ...
    fuzz_one_extras!(ctx, "PRESENT", "1", [], [(mode "PCIE_2_0")], extras);

    for &attr in PCIE_BOOL_ATTRS {
        fuzz_enum!(ctx, attr, ["FALSE", "TRUE"], [(mode "PCIE_2_0")]);
    }
    for &(attr, width) in PCIE_HEX_ATTRS {
        fuzz_multi_attr_hex!(ctx, attr, width, [(mode "PCIE_2_0")]);
    }
    for &(attr, width) in PCIE_DEC_ATTRS {
        fuzz_multi_attr_dec!(ctx, attr, width, [(mode "PCIE_2_0")]);
    }
}
