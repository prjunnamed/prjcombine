...
use prjcombine_interconnect::{
    db::{BelInfo, IntDb, SwitchBoxItem, WireKind, WireSlotId},
...
pub fn dump_chip(die: &Die, kind: ChipKind) -> (Chip, IntDb, NamingDb) {
    ...
    let mut intdb: IntDb =
        bincode::decode_from_slice(defs::xc3000::INIT, bincode::config::standard())
            .unwrap()
            .0;
        ...
        for &net_f in net_info.pips_bwd.keys() {
            ...
            if rwt.cell.col == rwf.cell.col {
                ...
                queue.push((
                    net_t,
                    net_f,
                    CellCoord::new(die, col, row).tile(defs::tslots::EXTRA_ROW),
                ))
            } else {
                ...
                queue.push((
                    net_t,
                    net_f,
                    CellCoord::new(die, col, row).tile(defs::tslots::EXTRA_COL),
                ))
            }
        }
    }
    ...
    for (net, net_info) in &extractor.nets {
        ...
        let idx = wires::IOCLK_W
            .index_of(rw.slot)
            .or(wires::IOCLK_E.index_of(rw.slot))
            .or(wires::IOCLK_S.index_of(rw.slot))
            .or(wires::IOCLK_N.index_of(rw.slot))
            .unwrap();
