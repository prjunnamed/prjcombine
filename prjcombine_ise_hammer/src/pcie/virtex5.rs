...
use crate::{
    ...
    fuzz_multi_attr_dec, fuzz_multi_attr_hex, fuzz_one,
};
...
pub fn add_fuzzers<'a>(session: &mut Session<IseBackend<'a>>, backend: &IseBackend<'a>) {
    ...
    fuzz_one!(ctx, "PRESENT", "1", [], [(mode "PCIE")]);

    for &pin in PCIE_INVPINS {
        fuzz_inv!(ctx, pin, [(mode "PCIE")]);
    }
    for &attr in PCIE_BOOL_ATTRS {
        fuzz_enum!(ctx, attr, ["FALSE", "TRUE"], [(mode "PCIE")]);
    }
    for &(attr, width) in PCIE_HEX_ATTRS {
        fuzz_multi_attr_hex!(ctx, attr, width, [(mode "PCIE")]);
    }
    for &(attr, width) in PCIE_DEC_ATTRS {
        fuzz_multi_attr_dec!(ctx, attr, width, [(mode "PCIE")]);
    }
}
