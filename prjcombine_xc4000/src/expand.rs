    ...
    pub fn expand_grid<'a>(&'a self, db: &'a IntDb) -> ExpandedDevice<'a> {
            ...
            if col == self.col_lio() {
                for row in grid.rows() {
                    ...
                    if row == self.row_bio() {
                        ...
                        let node = self.get_lio_node(row);
                ...
                for row in grid.rows() {
                    ...
                    let r = row_t.to_idx() - row.to_idx();
                    if row == self.row_bio() {
                        let node = grid.fill_tile((col, row), "CNR.BR", "CNR.BR", name);
                        if self.kind == GridKind::SpartanXl {
                        ...
                        let node = self.get_rio_node(row);
                ...
                for row in grid.rows() {
                    ...
                    let r = row_t.to_idx() - row.to_idx();
                    if row == self.row_bio() {
                        let node = self.get_bio_node(col);
                        ...
                        let node = self.get_tio_node(col);
        ...
        if self.kind.is_xl() {
            ...
            for row in grid.rows() {
                ...
                for (lr, col) in [('L', self.col_ql()), ('R', self.col_qr())] {
                    ...
                    let (kind, naming, name) = if row == self.row_bio() {
                        ("LLHQ.IO.B", "LLHQ.IO.B", format!("BQ{lr}"))
                    } else if row == self.row_tio() {
                        ("LLHQ.IO.T", "LLHQ.IO.T", format!("TQ{lr}"))
                    } else {
                        (
                            if row == self.row_bio() + 1 {
                                "LLHQ.CLB.B"
                ...
                let (kind, naming, name) = if row == self.row_bio() {
                    ("LLHC.IO.B", "LLHC.IO.B", "BM".to_string())
                } else if row == self.row_tio() {
                    ("LLHC.IO.T", "LLHC.IO.T", "TM".to_string())
                } else {
                    (
                        if row == self.row_bio() + 1 {
                            "LLHC.CLB.B"
            ...
            for col in grid.cols() {
                ...
                for (bt, row) in [('B', self.row_qb()), ('T', self.row_qt())] {
                    ...
                    let (kind, naming, name) = if col == self.col_lio() {
                        ...
                        ("LLVQ.CLB", "LLVQ.CLB", format!("HQ{bt}C{c}"))
                    };
                    let node = grid.add_xnode(
                        (col, row),
                        db.get_node(kind),
                ...
                let (kind, name) = if col == self.col_lio() {
                    ("LLVC.IO.L", "LM".to_string())
                } else if col == self.col_rio() {
                    ("LLVC.IO.R", "RM".to_string())
                } else {
                    ("LLVC.CLB", format!("HMC{c}"))
                };
                let node = grid.add_xnode(
                    (col, row),
                    db.get_node(kind),
            ...
            for row in grid.rows() {
                ...
                grid.fill_term_pair_anon((col - 1, row), (col, row), llhc_e, llhc_w);
                let (kind, naming, name) = if row == self.row_bio() {
                    ("LLH.IO.B", "LLH.IO.B", "BM".to_string())
                } else if row == self.row_tio() {
                    ("LLH.IO.T", "LLH.IO.T", "TM".to_string())
                } else {
                    (
                        if row == self.row_bio() + 1 {
                            "LLH.CLB.B"
            ...
            for col in grid.cols() {
                ...
                let c = col.to_idx();
                let (kind, name) = if col == self.col_lio() {
                    ("LLV.IO.L", "LM".to_string())
                } else if col == self.col_rio() {
                    ("LLV.IO.R", "RM".to_string())
                } else {
                    ("LLV.CLB", format!("HMC{c}"))
                };
                grid.add_xnode(
                    (col, row),
                    db.get_node(kind),
