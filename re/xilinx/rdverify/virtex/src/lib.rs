...
fn verify_iob(endev: &ExpandedNamedDevice, vrf: &mut Verifier, bel: &BelContext<'_>) {
    ...
    if (bel.col == endev.grid.col_lio() || bel.col == endev.grid.col_rio())
        && ((bel.row == endev.grid.row_mid() && bel.key == "IOB3")
            || (bel.row == endev.grid.row_mid() - 1 && bel.key == "IOB1"))
    {
        kind = "PCIIOB";
...
fn verify_pcilogic(endev: &ExpandedNamedDevice, vrf: &mut Verifier, bel: &BelContext<'_>) {
    ...
    let obel = vrf
        .find_bel(bel.die, (bel.col, endev.grid.row_mid()), "IOB3")
        .unwrap();
    vrf.verify_node(&[bel.fwire_far("IRDY"), obel.fwire("PCI")]);
    let obel = vrf
        .find_bel(bel.die, (bel.col, endev.grid.row_mid() - 1), "IOB1")
        .unwrap();
...
fn verify_clkc(endev: &ExpandedNamedDevice, vrf: &mut Verifier, bel: &BelContext<'_>) {
    for (opin, ipin, srow, sbel) in [
        ("OUT0", "IN0", endev.grid.row_bio(), "BUFG0"),
        ("OUT1", "IN1", endev.grid.row_bio(), "BUFG1"),
        ("OUT2", "IN2", endev.grid.row_tio(), "BUFG0"),
        ("OUT3", "IN3", endev.grid.row_tio(), "BUFG1"),
    ] {
        vrf.claim_node(&[bel.fwire(opin)]);
        vrf.claim_pip(bel.crd(), bel.wire(opin), bel.wire(ipin));
        let obel = vrf
            .find_bel(bel.die, (endev.grid.col_clk(), srow), sbel)
            .unwrap();
...
fn verify_clkv(endev: &ExpandedNamedDevice, vrf: &mut Verifier, bel: &BelContext<'_>) {
    for (opinl, opinr, ipin, opin) in [
        ...
        let obel = vrf
            .find_bel(bel.die, (bel.col, endev.grid.row_clk()), "GCLKC")
            .unwrap();
        vrf.verify_node(&[bel.fwire(ipin), obel.fwire(opin)]);
    }
}
...
fn verify_bel(endev: &ExpandedNamedDevice, vrf: &mut Verifier, bel: &BelContext<'_>) {
    match bel.key {
        ...
        "TBUS" => verify_tbus(endev, vrf, bel),
        ...
        "PCILOGIC" => verify_pcilogic(endev, vrf, bel),
        "DLL" => vrf.verify_bel(bel, "DLL", &[], &[]),
        "CLKC" => verify_clkc(endev, vrf, bel),
        "GCLKC" => verify_gclkc(endev, vrf, bel),
        "BRAM_CLKH" => verify_bram_clkh(endev, vrf, bel),
        "CLKV" => verify_clkv(endev, vrf, bel),
