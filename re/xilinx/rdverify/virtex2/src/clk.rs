...
pub fn verify_bufgmux(endev: &ExpandedNamedDevice, vrf: &mut Verifier, bel: &BelContext) {
    if endev.edev.chip.kind == ChipKind::FpgaCore {
        ...
        let crd = endev.chip.get_clk_io(edge, bel.slot.to_idx()).unwrap();
    ...
    // vrf.claim_pip(bel.crd(), bel.wire_far("CLK"), obel.wire("VCCOUT"));
        ...
        if endev.chip.kind.is_virtex2() || endev.chip.kind == ChipKind::Spartan3 {
            if let Some(crd) = endev.chip.get_clk_io(edge, bel.slot.to_idx()) {
            ...
            vrf.claim_pip(obel.crd(), obel.wire("IBUF"), obel.wire("I"));
            let crd = endev.chip.get_clk_io(edge, bel.slot.to_idx() + 4).unwrap();
            let obel = get_bel_iob(endev, vrf, crd);
            ...
            if has_dcm_l {
                ...
                let (dcm_pad_pin, dcm_out_pin) = match (edge, bel.slot.to_idx()) {
            ...
            if endev.chip.dcms == Some(Dcms::Eight) {
                ...
                let srow = if bel.slot.to_idx() < 4 {
                    endev.chip.row_mid()
                } else {
                    endev.chip.row_mid() - 1
                };
                let obel = vrf.get_bel((bel.die, (scol, srow), bels::DCMCONN_S3E));
                let (dcm_pad_pin, dcm_out_pin) = match bel.slot.to_idx() {
                    0 | 4 => ("CLKPAD0", "OUT0"),
                    1 | 5 => ("CLKPAD1", "OUT1"),
                    2 | 6 => ("CLKPAD2", "OUT2"),
                    3 | 7 => ("CLKPAD3", "OUT3"),
...
pub fn verify_gclkh(endev: &ExpandedNamedDevice, vrf: &mut Verifier, bel: &BelContext) {
    for i in 0..8 {
        for bt in ["B", "T"] {
            if matches!((bel.slot, bt), (bels::GCLKH_S, "T") | (bels::GCLKH_N, "B")) {
                continue;
            }
            vrf.claim_pip(
                bel.crd(),
