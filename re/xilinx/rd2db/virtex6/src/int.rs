...
impl IntMaker<'_> {
    fn fill_int_wires(&mut self) {
        ...
        self.builder
            .wire_names(wires::TIE_0, &["GND_WIRE", "DSP_GND"]);
        self.builder
            .wire_names(wires::TIE_1, &["VCC_WIRE", "DSP_VCC"]);

        for i in 0..8 {
            self.builder
                .wire_names(wires::LCLK[i], &[format!("GCLK_B{i}")]);
        }
        ...
        for i in 0..17 {
            ...
            self.builder.wire_names(wires::LV[i], &[format!("LV{i}")]);
        }
        ...
        for i in 0..2 {
            self.builder
                .wire_names(wires::IMUX_CLK[i], &[format!("CLK_B{i}")]);
        }
        for i in 0..2 {
            self.builder
                .wire_names(wires::IMUX_CTRL[i], &[format!("CTRL_B{i}")]);
        }
        for i in 0..8 {
            self.builder
                .wire_names(wires::IMUX_BYP[i], &[format!("BYP{i}")]);
            self.builder.mark_permabuf(wires::IMUX_BYP_SITE[i]);
            self.builder.mark_permabuf(wires::IMUX_BYP_BOUNCE[i]);
            self.builder
                .wire_names(wires::IMUX_BYP_SITE[i], &[format!("BYP_B{i}")]);
            self.builder
        ...
        for i in 0..48 {
            self.builder
                .wire_names(wires::IMUX_IMUX[i], &[format!("IMUX_B{i}")]);
            self.builder
        ...
        for i in 0..24 {
            self.builder
                .wire_names(wires::OUT[i], &[format!("LOGIC_OUTS{i}")]);
            self.builder
    ...
    fn fill_hclk_wires(&mut self) {
        for i in 0..12 {
            ...
            self.builder
                .extra_name_sub(format!("HCLK_QBUF_CK_BUFHCLK{i}"), 1, wires::HCLK_ROW[i]);
            self.builder
                .extra_name_sub(format!("HCLK_IOI_CK_BUFHCLK{i}"), 4, wires::HCLK_ROW[i]);
            self.builder
                .extra_name_sub(format!("HCLK_CMT_CK_HCLK_L{i}"), 20, wires::HCLK_CMT_W[i]);
            self.builder
                .extra_name_sub(format!("HCLK_CMT_CK_HCLK_R{i}"), 20, wires::HCLK_CMT_E[i]);

            self.builder
                .extra_name_sub(format!("HCLK_IOI_LEAF_GCLK{i}"), 4, wires::HCLK_IO[i]);
            self.builder
                .extra_name_sub(format!("IOI_LEAF_GCLK{i}"), 0, wires::HCLK_IO[i]);
        }
        for i in 0..6 {
            self.builder
                .extra_name_sub(format!("HCLK_CK_BUFRCLK{i}"), 1, wires::RCLK_ROW[i]);
            self.builder
                .extra_name_sub(format!("HCLK_QBUF_CK_BUFRCLK{i}"), 1, wires::RCLK_ROW[i]);
            self.builder
                .extra_name_sub(format!("HCLK_IOI_CK_BUFRCLK{i}"), 4, wires::RCLK_ROW[i]);
            self.builder
                .extra_name_sub(format!("HCLK_CMT_CK_RCLK_L{i}"), 20, wires::RCLK_CMT_W[i]);
            self.builder
                .extra_name_sub(format!("HCLK_CMT_CK_RCLK_R{i}"), 20, wires::RCLK_CMT_E[i]);

            self.builder
                .extra_name_sub(format!("HCLK_IOI_RCLK_TO_IO{i}"), 4, wires::RCLK_IO[i]);
            self.builder
                .extra_name_sub(format!("IOI_RCLK_FORIO{i}"), 0, wires::RCLK_IO[i]);
        }
        for i in 0..10 {
            self.builder
                .extra_name_sub(format!("HCLK_IOI_CK_MGT{i}"), 4, wires::MGT_ROW[i]);
            self.builder
                .extra_name_sub(format!("HCLK_CMT_CK_MGT_L{i}"), 20, wires::MGT_CMT_W[i]);
            self.builder
                .extra_name_sub(format!("HCLK_CMT_CK_MGT_R{i}"), 20, wires::MGT_CMT_E[i]);
            self.builder
                .extra_name_sub(format!("HCLK_GTX_MGT{i}"), 20, wires::MGT_ROW[i]);
            self.builder
                .extra_name_sub(format!("HCLK_GTX_LEFT_MGT{i}"), 20, wires::MGT_ROW[i]);
            self.builder
                .extra_name_sub(format!("HCLK_GTH_LEFT_MGT{i}"), 20, wires::MGT_ROW[i]);
            self.builder
                .extra_name_sub(format!("HCLK_GTH_RIGHT_MGT{i}"), 20, wires::MGT_ROW[i]);
        }
    }

    fn fill_io_wires(&mut self) {
        self.builder
            .alt_name_sub("HCLK_IOI_RCLK_TOP0", 4, wires::IMUX_BUFIO[0]);
        self.builder
            .alt_name_sub("HCLK_IOI_RCLK_TOP1", 4, wires::IMUX_BUFIO[1]);
        self.builder
            .alt_name_sub("HCLK_IOI_RCLK_BOT0", 4, wires::IMUX_BUFIO[2]);
        self.builder
            .alt_name_sub("HCLK_IOI_RCLK_BOT1", 4, wires::IMUX_BUFIO[3]);
        self.builder
            .extra_name_sub("HCLK_IOI_RCLK_IMUX_TOP_B", 3, wires::IMUX_IMUX[4]);
        self.builder
            .extra_name_sub("HCLK_IOI_RCLK_IMUX_BOT_B", 4, wires::IMUX_IMUX[4]);

        self.builder
            .extra_name_sub("HCLK_IOI_RCLK_TOP_BEFORE_DIV", 4, wires::IMUX_BUFR[0]);
        self.builder
            .extra_name_sub("HCLK_IOI_RCLK_BOT_BEFORE_DIV", 4, wires::IMUX_BUFR[1]);
        ...
        self.builder
            .extra_name_sub("HCLK_GTX_PERFCLK", 20, wires::IMUX_GTX_PERFCLK);
        self.builder
            .extra_name_sub("HCLK_GTX_LEFT_PERFCLK", 20, wires::IMUX_GTX_PERFCLK);

        for i in 0..4 {
            self.builder
                .extra_name_sub(format!("HCLK_IOI_IOCLK{i}"), 4, wires::IOCLK[i]);
            self.builder
                .extra_name_sub(format!("HCLK_IOI_IOCLKMULTI{i}"), 4, wires::IOCLK[i + 4]);
        }
        for i in 0..8 {
            self.builder
        ...
        self.builder
            .extra_name_sub("HCLK_IOI_VIOCLK0", 4, wires::VIOCLK[0]);
        self.builder
            .extra_name_sub("HCLK_IOI_VIOCLK1", 4, wires::VIOCLK[1]);
        self.builder
            .extra_name_sub("HCLK_IOI_SIOCLK1", 4, wires::SIOCLK[0]);
        self.builder
            .extra_name_sub("HCLK_IOI_SIOCLK2", 4, wires::SIOCLK[1]);
        self.builder
            .extra_name_sub("HCLK_IOI_VIOCLK_SOUTH0", 4, wires::VIOCLK_S_BUF[0]);
        self.builder
            .extra_name_sub("HCLK_IOI_VIOCLK_SOUTH1", 4, wires::VIOCLK_S_BUF[1]);
        self.builder
            .extra_name_sub("HCLK_IOI_VIOCLK_NORTH0", 4, wires::VIOCLK_N_BUF[0]);
        self.builder
            .extra_name_sub("HCLK_IOI_VIOCLK_NORTH1", 4, wires::VIOCLK_N_BUF[1]);
        for i in 0..2 {
            self.builder
                .extra_name_sub(format!("HCLK_IOI_VRCLK{i}"), 4, wires::VRCLK[i]);
            self.builder
                .extra_name_sub(format!("HCLK_IOI_VRCLK_SOUTH{i}"), 4, wires::VRCLK_S[i]);
            self.builder
                .extra_name_sub(format!("HCLK_IOI_VRCLK_NORTH{i}"), 4, wires::VRCLK_N[i]);

            self.builder
                .extra_name_sub(format!("HCLK_IOI_BUFOCLK{i}"), 4, wires::VOCLK[i]);
            self.builder.extra_name_sub(
                format!("HCLK_IOI_VBUFOCLK_SOUTH{i}"),
                4,
                wires::VOCLK_S[i],
            );
            self.builder.extra_name_sub(
                format!("HCLK_IOI_VBUFOCLK_NORTH{i}"),
                4,
                wires::VOCLK_N[i],
            );
            self.builder.alt_name_sub(
            ...
            self.builder
                .extra_name_sub(format!("HCLK_IOI_BUFO_IN{i}"), 4, wires::OCLK[i]);
            self.builder
                .extra_name(format!("IOI_BUFO_CLK{i}"), wires::OCLK[i]);
        }
        for i in 0..2 {
    ...
    fn fill_cmt_wires(&mut self) {
        for i in 0..4 {
            self.builder
                .extra_name_sub(format!("CMT_CK_PERF_INNER_L{i}"), 44, wires::PERF_ROW[i]);
            self.builder.extra_name_sub(
            ...
            self.builder
                .extra_name_sub(format!("CMT_CK_PERF_INNER_R{i}"), 52, wires::PERF_ROW[i]);
            self.builder.extra_name_sub(
            ...
            self.builder
                .extra_name_sub(format!("HCLK_GTX_PERF_OUTER{i}"), 20, wires::PERF_ROW[i]);
            self.builder.extra_name_sub(
        ...
        for i in 0..4 {
            self.builder
                .extra_name_sub(format!("HCLK_CMT_CK_CCIO_L{i}"), 20, wires::CCIO_CMT_W[i]);
            self.builder
                .extra_name_sub(format!("HCLK_CMT_CK_CCIO_R{i}"), 20, wires::CCIO_CMT_E[i]);
        ...
        for i in 0..8 {
            self.builder
                .extra_name(format!("CMT_BUFG_BOT_CK_IO_TO_BUFG{i}"), wires::GIOB[i]);
            self.builder
                .extra_name(format!("CMT_BUFG_TOP_CK_IO_TO_BUFG{i}"), wires::GIOB[i]);
            self.builder.mark_permabuf(wires::GIOB[i]);
            self.builder.extra_name_sub(
                format!("HCLK_CMT_CK_IO_TO_CMT{i}"),
                20,
                wires::GIOB_CMT[i],
            );
        }
        for (tkn, c) in [("CMT_BUFG_BOT", 1), ("CMT_BUFG_TOP", 0)] {
            ...
            for i in 0..32 {
                self.builder.mark_permabuf(wires::GCLK[i]);
                self.builder.extra_name_tile_sub(
                    tkn,
        ...
        for i in 0..32 {
            self.builder
                .extra_name_sub(format!("HCLK_CMT_CK_GCLK{i}"), 20, wires::GCLK_CMT[i]);
            self.builder
                .extra_name(format!("CMT_BUFG_CK_GCLK{i}"), wires::GCLK[i]);
            ...
            self.builder
                .extra_name(format!("CMT_BUFG_TOP_CK_MUXED{i}"), wires::IMUX_BUFG_I[i]);
            self.builder.extra_name_sub(
        ...
        for i in 0..12 {
            self.builder.extra_name_sub(
                format!("HCLK_CMT_CK_OUT_L{i}"),
                20,
                wires::IMUX_BUFHCE_W[i],
            );
            self.builder.extra_name_sub(
                format!("HCLK_CMT_CK_OUT_R{i}"),
                20,
        ...
        for i in 0..14 {
            self.builder
                .extra_name_sub(format!("HCLK_CMT_CK_CMT_BOT{i}"), 20, wires::OUT_PLL_S[i]);
            self.builder
                .extra_name_sub(format!("HCLK_CMT_CK_CMT_TOP{i}"), 20, wires::OUT_PLL_N[i]);
        }
        for i in 0..32 {
            self.builder.extra_name_sub(
        ...
        for (tkn, outs) in [("CMT_BOT", wires::OUT_PLL_S), ("CMT_TOP", wires::OUT_PLL_N)] {
            self.builder
                .extra_name_tile_sub(tkn, "CMT_MMCM_CLKOUT0", 20, outs[0]);
            self.builder
                .extra_name_tile_sub(tkn, "CMT_MMCM_CLKOUT0B", 20, outs[1]);
            self.builder
                .extra_name_tile_sub(tkn, "CMT_MMCM_CLKOUT1", 20, outs[2]);
            self.builder
                .extra_name_tile_sub(tkn, "CMT_MMCM_CLKOUT1B", 20, outs[3]);
            self.builder
                .extra_name_tile_sub(tkn, "CMT_MMCM_CLKOUT2", 20, outs[4]);
            self.builder
                .extra_name_tile_sub(tkn, "CMT_MMCM_CLKOUT2B", 20, outs[5]);
            self.builder
                .extra_name_tile_sub(tkn, "CMT_MMCM_CLKOUT3", 20, outs[6]);
            self.builder
                .extra_name_tile_sub(tkn, "CMT_MMCM_CLKOUT3B", 20, outs[7]);
            self.builder
                .extra_name_tile_sub(tkn, "CMT_MMCM_CLKOUT4", 20, outs[8]);
            self.builder
                .extra_name_tile_sub(tkn, "CMT_MMCM_CLKOUT5", 20, outs[9]);
            self.builder
                .extra_name_tile_sub(tkn, "CMT_MMCM_CLKOUT6", 20, outs[10]);
            self.builder
                .extra_name_tile_sub(tkn, "CMT_MMCM_CLKFBOUT", 20, outs[11]);
            self.builder
                .extra_name_tile_sub(tkn, "CMT_MMCM_CLKFBOUTB", 20, outs[12]);
            self.builder
                .extra_name_tile_sub(tkn, "CMT_MMCM_TMUXOUT", 20, outs[13]);
        }
        for i in 0..4 {
            self.builder.extra_name_tile_sub(
                "CMT_BOT",
        ...
        self.builder.extra_name_tile_sub(
            ...
            "CMT_MMCM_CASC_OUT",
            20,
            wires::OMUX_PLL_CASC[0],
        );
        self.builder.extra_name_tile_sub(
            "CMT_TOP",
            "CMT_MMCM_CASC_OUT",
            20,
            wires::OMUX_PLL_CASC[1],
        );
        self.builder.extra_name_tile_sub(
            "CMT_BOT",
            "CMT_MMCM_CLKIN1",
            20,
            wires::IMUX_PLL_CLKIN1[0],
        );
        self.builder.extra_name_tile_sub(
            "CMT_TOP",
            "CMT_MMCM_CLKIN1",
        ...
        self.builder
            .extra_name_tile_sub("CMT_BOT", "CMT_MMCM_IMUX_CLKIN1", 17, wires::IMUX_CLK[0]);
        self.builder
            .extra_name_tile_sub("CMT_TOP", "CMT_MMCM_IMUX_CLKIN1", 22, wires::IMUX_CLK[1]);
        self.builder
            .extra_name_tile_sub("CMT_BOT", "CMT_MMCM_IMUX_CLKIN2", 17, wires::IMUX_CLK[1]);
        self.builder
            .extra_name_tile_sub("CMT_TOP", "CMT_MMCM_IMUX_CLKIN2", 22, wires::IMUX_CLK[0]);
        self.builder
            .extra_name_tile_sub("CMT_BOT", "CMT_MMCM_IMUX_CLKFB", 18, wires::IMUX_CLK[1]);
        self.builder
            .extra_name_tile_sub("CMT_TOP", "CMT_MMCM_IMUX_CLKFB", 21, wires::IMUX_CLK[0]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLK_0_B0", 20, wires::BUFH_INT_W[0]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLK_0_B1", 20, wires::BUFH_INT_W[1]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLK_1_B0", 20, wires::BUFH_INT_E[0]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLK_1_B1", 20, wires::BUFH_INT_E[1]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKFB_HCLK_B", 20, wires::IMUX_PLL_CLKFB_HCLK[0]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKFB_HCLK_T", 20, wires::IMUX_PLL_CLKFB_HCLK[1]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKFB_IO_B", 20, wires::IMUX_PLL_CLKFB_IO[0]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKFB_IO_T", 20, wires::IMUX_PLL_CLKFB_IO[1]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKIN1_HCLK_B", 20, wires::IMUX_PLL_CLKIN1_HCLK[0]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKIN1_HCLK_T", 20, wires::IMUX_PLL_CLKIN1_HCLK[1]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKIN2_HCLK_B", 20, wires::IMUX_PLL_CLKIN2_HCLK[0]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKIN2_HCLK_T", 20, wires::IMUX_PLL_CLKIN2_HCLK[1]);
        self.builder.extra_name_sub(
        ...
        self.builder
            .extra_name_sub("HCLK_CMT_CLKIN1_IO_B", 20, wires::IMUX_PLL_CLKIN1_IO[0]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKIN1_IO_T", 20, wires::IMUX_PLL_CLKIN1_IO[1]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKIN2_IO_B", 20, wires::IMUX_PLL_CLKIN2_IO[0]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKIN2_IO_T", 20, wires::IMUX_PLL_CLKIN2_IO[1]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKIN1_MGT_B", 20, wires::IMUX_PLL_CLKIN1_MGT[0]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKIN1_MGT_T", 20, wires::IMUX_PLL_CLKIN1_MGT[1]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKIN2_MGT_B", 20, wires::IMUX_PLL_CLKIN2_MGT[0]);
        self.builder
            .extra_name_sub("HCLK_CMT_CLKIN2_MGT_T", 20, wires::IMUX_PLL_CLKIN2_MGT[1]);
        self.builder
            .extra_name_sub("HCLK_CMT_CK_BUFH_TEST_L", 20, wires::BUFH_TEST_W);
        self.builder
            .extra_name_sub("HCLK_CMT_CK_BUFH_TEST_R", 20, wires::BUFH_TEST_E);
        self.builder
            .extra_name_sub("HCLK_CMT_CK_BUFH_TEST_OUT_L", 20, wires::BUFH_TEST_W_IN);
        self.builder
            .extra_name_sub("HCLK_CMT_CK_BUFH_TEST_OUT_R", 20, wires::BUFH_TEST_E_IN);
        self.builder
            .mark_optinv(wires::BUFH_TEST_W_IN, wires::BUFH_TEST_W);
        self.builder
            .mark_optinv(wires::BUFH_TEST_E_IN, wires::BUFH_TEST_E);
    }

    fn fill_dsp_wires(&mut self) {
        let tk = self.rd.tile_kinds.get("DSP").unwrap().1;
        for &(wf, wt) in tk.pips.keys() {
            let wf = self.rd.wires[wf].as_str();
            let wt = self.rd.wires[wt].as_str();
            if let Some(suf) = wf.strip_prefix("DSP_BYP_B") {
                let (idx, cell) = suf.split_once('_').unwrap();
                let idx: usize = idx.parse().unwrap();
                let cell: usize = cell.parse().unwrap();
                self.builder
                    .extra_name_sub(wt, cell, wires::IMUX_BYP_DSP[idx]);
            } else if let Some(suf) = wf.strip_prefix("DSP_FAN_B") {
                let (idx, cell) = suf.split_once('_').unwrap();
                let idx: usize = idx.parse().unwrap();
                let cell: usize = cell.parse().unwrap();
                self.builder
    ...
    fn fill_int_tiles(&mut self) {
        ...
        self.builder
            .extract_term_conn_id(ccls::TERM_W, Dir::W, "L_TERM_INT", &[]);
        self.builder
            .extract_term_conn_id(ccls::TERM_E, Dir::E, "R_TERM_INT", &[]);
        self.builder
            .extract_term_conn_id(ccls::TERM_S, Dir::S, "BRKH_T_TERM_INT", &[]);
        for &xy in self.rd.tiles_by_kind_name("PCIE") {
            let int_xy_a = Coord {
                x: xy.x,
                y: xy.y + 11,
            };
            ...
            self.builder
                .extract_term_conn_tile_id(ccls::TERM_S, Dir::S, int_xy_a, &[]);
            self.builder
                .extract_term_conn_tile_id(ccls::TERM_S, Dir::S, int_xy_b, &[]);
        }
        self.builder
            .extract_term_conn_id(ccls::TERM_N, Dir::N, "BRKH_B_TERM_INT", &[]);
    ...
    fn fill_bram_tiles(&mut self) {
        if let Some(&xy) = self.rd.tiles_by_kind_name("BRAM").iter().next() {
            let mut int_xy = Vec::new();
            let mut intf_xy = Vec::new();
            let n = self.builder.ndb.get_tile_class_naming("INTF");
            for dy in 0..5 {
                int_xy.push(Coord {
                    x: xy.x - 2,
                    y: xy.y + dy,
            ...
            let bel_bram_h0 = self.builder.bel_xy(bslots::BRAM_H[0], "RAMB18", 0, 0);
            ...
            let bels = [bel_bram_f, bel_bram_h0, bel_bram_h1];
            let mut x = self
                .builder
    ...
    fn fill_dsp_tiles(&mut self) {
        if let Some(&xy) = self.rd.tiles_by_kind_name("DSP").iter().next() {
            let mut int_xy = Vec::new();
            let mut intf_xy = Vec::new();
            let n = self.builder.ndb.get_tile_class_naming("INTF");
            for dy in 0..5 {
    ...
    fn fill_emac_tiles(&mut self) {
        if let Some(&xy) = self.rd.tiles_by_kind_name("EMAC").iter().next() {
            let mut int_xy = Vec::new();
            let mut intf_xy = Vec::new();
            let n = self.builder.ndb.get_tile_class_naming("INTF_EMAC");
            for dy in 0..10 {
                int_xy.push(Coord {
                    x: xy.x - 2,
                    y: xy.y + dy,
            ...
            let bel = self.builder.bel_xy(bslots::EMAC, "TEMAC", 0, 0);
            let mut x = self
                .builder
                .xtile_id(tcls::EMAC, "EMAC", xy)
                .num_cells(10)
                .bel(bel);
            for (i, &xy) in int_xy.iter().enumerate() {
                x = x.ref_int(xy, i);
    ...
    fn fill_pcie_tiles(&mut self) {
        if let Some(&xy) = self.rd.tiles_by_kind_name("PCIE").iter().next() {
            let mut int_xy = Vec::new();
            let mut intf_xy = Vec::new();
            let nl = self.builder.ndb.get_tile_class_naming("INTF_PCIE_L");
            let nr = self.builder.ndb.get_tile_class_naming("INTF_PCIE_R");
            for dy in 0..20 {
                int_xy.push(Coord {
                    x: xy.x - 4,
                    y: xy.y - 10 + dy,
    ...
    fn fill_hclk_tiles(&mut self) {
        for (tkn, naming) in [
            ...
            if let Some(&xy) = self.rd.tiles_by_kind_name(tkn).iter().next() {
                let bel_gsig = self.builder.bel_xy(bslots::GLOBALSIG, "GLOBALSIG", 0, 0);
                let bel_drp = self.builder.bel_virtual(bslots::HCLK_DRP[0]);
                self.builder
    ...
    fn fill_io_tiles(&mut self) {
        for (tkn, naming_l, naming_r) in [
            ...
            for &xy in self.rd.tiles_by_kind_name(tkn) {
                let is_l =
                    self.rd.tile_kinds.key(self.rd.tiles[&xy.delta(-1, 0)].kind) == "HCLK_IOB";
                let hclk_xy = if is_l {
                ...
                let intf_io = self.builder.ndb.get_tile_class_naming(if is_l {
                    "INTF_IOI_L"
                } else {
                    "INTF"
                });
                let mut bels = vec![];
                for i in 0..4 {
                    bels.push(self.builder.bel_xy(bslots::BUFIO[i], "BUFIODQS", 0, i ^ 2));
                }
                for i in 0..2 {
                    bels.push(self.builder.bel_xy(bslots::BUFR[i], "BUFR", 0, i ^ 1));
                }
            ...
            for nn in [naming_l, naming_r] {
                let Some((_, naming)) = self.builder.ndb.tile_class_namings.get_mut(nn) else {
                    continue;
                };
                for i in 0..8 {
        ...
        for tkn in ["LIOI", "RIOI"] {
            if let Some(&xy) = self.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                let int_xy = if is_l {
                    self.builder.walk_to_int(xy, Dir::E, false).unwrap()
                } else {
                    self.builder.walk_to_int(xy, Dir::W, false).unwrap()
                };
                let intf_io = self.builder.ndb.get_tile_class_naming(if is_l {
                    "INTF_IOI_L"
                } else {
                ...
                for i in 0..2 {
                    ...
                    if i == 1 {
                        bel = bel
                            ...
                            .extra_wire_force(
                                "CLKPAD_CMT",
                                format!("{lr}IOI_I_2IOCLK_BOT1_I2GCLK"),
                            );
                    }
                    bels.push(bel);
                }
        ...
        let pips = self
            .builder
            .pips
            .get_mut(&(tcls::IO, bslots::SPEC_INT))
            .unwrap();
        for (&(wt, _), mode) in pips.pips.iter_mut() {
    ...
    fn fill_cfg_tiles(&mut self) {
        if let Some(&xy) = self.rd.tiles_by_kind_name("CFG_CENTER_0").iter().next() {
            let intf = self.builder.ndb.get_tile_class_naming("INTF");
            let mut bel_sysmon = self
                .builder
            ...
            let bels = [
                ...
                self.builder
                    ...
                    .raw_tile(1),
                self.builder
                    .bel_xy(bslots::BSCAN[2], "BSCAN", 0, 0)
                    .raw_tile(2),
                self.builder
                    .bel_xy(bslots::BSCAN[3], "BSCAN", 0, 1)
                    .raw_tile(2),
                self.builder
                    .bel_xy(bslots::ICAP[0], "ICAP", 0, 0)
                    .raw_tile(1),
                self.builder
                    .bel_xy(bslots::ICAP[1], "ICAP", 0, 0)
                    .raw_tile(2),
                self.builder
                    .bel_xy(bslots::PMV_CFG[0], "PMV", 0, 0)
                    .raw_tile(0),
                self.builder
                    .bel_xy(bslots::PMV_CFG[1], "PMV", 0, 0)
                    .raw_tile(3),
                self.builder
    ...
    fn fill_cmt_tiles(&mut self) {
        for (tkn, naming) in [("HCLK_CMT_BOT", "CMT.BOT"), ("HCLK_CMT_TOP", "CMT.TOP")] {
            if let Some(&xy) = self.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                for i in 0..2 {
                    ...
                    for j in 0..12 {
                        bels.push(self.builder.bel_xy(slots[j], "BUFHCE", i, j).raw_tile(2));
                    }
                }
                for i in 0..2 {
                    bels.push(
        ...
        for tkn in ["CMT_PMVA", "CMT_PMVA_BELOW"] {
            if let Some(&xy) = self.rd.tiles_by_kind_name(tkn).iter().next() {
                let intf = self.builder.ndb.get_tile_class_naming("INTF");
                let bel = self.builder.bel_xy(bslots::PMVIOB_CLK, "PMVIOB", 0, 0);
                self.builder
        ...
        for (tcid, naming, tkn) in [
            ...
            if let Some(&xy) = self.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                for i in 0..16 {
                    let ii = bi + i;
                    bels.push(self.builder.bel_xy(bslots::BUFGCTRL[ii], "BUFGCTRL", 0, i));
                }
                let mut bel = self.builder.bel_virtual(bslots::SPEC_INT).naming_only();
                for i in 0..8 {
                    bel = bel.extra_wire(
                        format!("GIO{i}_BUFG"),
                        &[
                ...
                for i in 0..4 {
                    bels.push(
                        self.builder
                            ...
                            .sub_xy(self.rd, "IPAD", 0, 1)
                            .raw_tile(i + 1)
                            .pin_rename("O", "IPAD_RXP_O")
                            .pins_name_only(&["IPAD_RXP_O"])
                            .sub_xy(self.rd, "IPAD", 0, 0)
                            .raw_tile(i + 1)
                            .pin_rename("O", "IPAD_RXN_O")
                            .pins_name_only(&["IPAD_RXN_O"])
                            .sub_xy(self.rd, "OPAD", 0, 1)
                            .raw_tile(i + 1)
                            .pin_rename("I", "OPAD_TXP_I")
                            .pins_name_only(&["OPAD_TXP_I"])
                            .sub_xy(self.rd, "OPAD", 0, 0)
                            .raw_tile(i + 1)
                            .pin_rename("I", "OPAD_TXN_I")
                            .pins_name_only(&["OPAD_TXN_I"]),
                    );
                }
                bels.extend([
                    self.builder
                        .bel_xy(bslots::GTCLK[0], "IBUFDS_GTXE1", 0, 0)
                        .pins_name_only(&["O", "ODIV2", "I", "IB"])
                        .extra_int_out(
                            "CLKOUT",
                    ...
                    self.builder
                        .bel_xy(bslots::GTCLK[1], "IBUFDS_GTXE1", 0, 1)
                        .pins_name_only(&["O", "ODIV2", "I", "IB"])
                        .extra_int_out(
                            "CLKOUT",
    ...
    fn fill_gth_tiles(&mut self) {
        for tkn in ["HCLK_GTH_LEFT", "HCLK_GTH"] {
            if let Some(&xy) = self.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                bel = bel
                    .sub_xy(self.rd, "IBUFDS_GTHE1", 0, 0)
                    .raw_tile(2)
                    .pin_rename("O", "BUFDS_O")
                    .pin_rename("I", "BUFDS_I")
                    .pin_rename("IB", "BUFDS_IB")
                    .pins_name_only(&["BUFDS_I", "BUFDS_IB"])
                    .pin_name_only("BUFDS_O", 1)
                    .sub_xy(self.rd, "IPAD", 0, 1)
                    .raw_tile(2)
                    .pin_rename("O", "IPAD_BUFDS_I_O")
                    .pins_name_only(&["IPAD_BUFDS_I_O"])
                    .sub_xy(self.rd, "IPAD", 0, 0)
                    .raw_tile(2)
                    .pin_rename("O", "IPAD_BUFDS_IB_O")
                    .pins_name_only(&["IPAD_BUFDS_IB_O"]);
                for i in 0..4 {
                    let rxp = &format!("IPAD_RXP{i}_O");
                    let rxn = &format!("IPAD_RXN{i}_O");
                    bel = bel
                        .sub_xy(self.rd, "IPAD", 0, (3 - i) * 2 + 1)
                        .pin_rename("O", rxp)
                        .raw_tile(1)
                        .pins_name_only(&[rxp])
                        .sub_xy(self.rd, "IPAD", 0, (3 - i) * 2)
                        .pin_rename("O", rxn)
                        .raw_tile(1)
                        .pins_name_only(&[rxn]);
                }
                for i in 0..4 {
                    let txp = &format!("OPAD_TXP{i}_I");
                    let txn = &format!("OPAD_TXN{i}_I");
                    bel = bel
                        .sub_xy(self.rd, "OPAD", 0, (3 - i) * 2 + 1)
                        .pin_rename("I", txp)
                        .raw_tile(1)
                        .pins_name_only(&[txp])
                        .sub_xy(self.rd, "OPAD", 0, (3 - i) * 2)
                        .pin_rename("I", txn)
                        .raw_tile(1)
                        .pins_name_only(&[txn]);
                }
