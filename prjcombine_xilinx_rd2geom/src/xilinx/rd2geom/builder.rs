...
use super::cfg::{GeomBuilderConfig, IntBufInfo, IntDoubleBufInfo, IntTermInfo, RawSiteSlot};
use super::part::PartBuilder;
use crate::namevec::NameVec;
use crate::xilinx::geomdb::builder::GeomDbBuilder;
use crate::xilinx::geomdb::{
    Dir, GeomDb, Grid, Orient, Part, SiteSlot, TCWire, TileMultiMux, TileMux, TileTran,
};
use crate::xilinx::geomraw::{ExtractClass, GeomRaw};
...
impl GeomBuilder {
    ...
    pub fn setup_int(&mut self) {
        ...
        let (pcls_int_n_pass, pcls_int_s_pass) = self
            .geomdb
            .make_port_pair(("INT_N_PASS", "INT_S_PASS"), (pslot_int_n, pslot_int_s));
        let (pcls_int_e_pass, pcls_int_w_pass) = self
            .geomdb
            .make_port_pair(("INT_E_PASS", "INT_W_PASS"), (pslot_int_e, pslot_int_w));
        ...
        for name in vec_buf_tcls {
            ...
            pcls_buf.insert(
                name,
                self.geomdb
                    .make_port_pair((&name_a, &name_b), (pslot_a, pslot_b)),
            );
            if needs_tile {
                self.geomdb.make_tile_single(name, tslot.unwrap());
            }
    ...
    pub fn connect_int_wire(&mut self, wire: usize, from_dir: Dir, from_wire: usize) {
        ...
        self.geomdb
            .make_simple_pconn(wire, from_wire, pcls_down, pcls_up);
    }
    ...
    pub fn finish(self) -> (GeomDb, GeomRaw) {
        for i in 0..self.geomdb.tiles.len() {
            if !self.filled_tcls.contains(&i) {
                println!(
                    "WARNING: unfinished tile class {}",
                    self.geomdb.tiles[i].name
                );
            }
        }
        for i in 0..self.geomdb.ports.len() {
            if !self.filled_pcls.contains(&i) {
                println!(
                    "WARNING: unfinished port class {}",
                    self.geomdb.ports[i].name
                );
            }
        }
