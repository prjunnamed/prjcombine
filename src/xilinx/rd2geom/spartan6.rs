...
impl Spartan6GeomMaker {
    ...
    fn setup_int_quad(&mut self) {
        for (name, da, db, dend) in [
            ...
            for i in 0..4 {
                ...
                let m = self.builder.make_int_wire(&format!("INT.{}M{}", name, i), "INT.QUAD", &[&format!("{}M{}", name, i)]);
                let c = self.builder.make_int_wire(&format!("INT.{}C{}", name, i), "INT.QUAD", &[&format!("{}C{}", name, i)]);
                let e = self.builder.make_int_wire(&format!("INT.{}E{}", name, i), "INT.QUAD", &[&format!("{}E{}", name, i)]);
                ...
                if i == 0 && dend == Some(Dir::S) {
                    let e_s = self.builder.make_int_wire(&format!("INT.{}E_S{}", name, i), "INT.QUAD", &[&format!("{}E_S{}", name, i)]);
                    self.builder.connect_int_wire(e_s, Dir::S, e);
                }
                if i == 3 && dend == Some(Dir::N) {
                    let e_n = self.builder.make_int_wire(&format!("INT.{}E_N{}", name, i), "INT.QUAD", &[&format!("{}E_N{}", name, i)]);
    ...
    fn setup_int_dbl(&mut self) {
        for (name, da, db, dend) in [
            ...
            for i in 0..4 {
                let b = self.builder.make_int_wire(&format!("INT.{}B{}", name, i), "INT.DBL", &[&format!("{}B{}", name, i)]);
                let m = self.builder.make_int_wire(&format!("INT.{}M{}", name, i), "INT.DBL", &[&format!("{}M{}", name, i)]);
                let e = self.builder.make_int_wire(&format!("INT.{}E{}", name, i), "INT.DBL", &[&format!("{}E{}", name, i)]);
                self.builder.connect_int_wire(m, da, b);
                self.builder.connect_int_wire(e, db, m);
                if i == 0 && dend == Some(Dir::S) {
                    let e_s = self.builder.make_int_wire(&format!("INT.{}E_S{}", name, i), "INT.DBL", &[&format!("{}E_S{}", name, i)]);
                    self.builder.connect_int_wire(e_s, Dir::S, e);
                }
                if i == 3 && dend == Some(Dir::N) {
                    let e_n = self.builder.make_int_wire(&format!("INT.{}E_N{}", name, i), "INT.DBL", &[&format!("{}E_N{}", name, i)]);
                    self.builder.connect_int_wire(e_n, Dir::N, e);
                }
            }
        }
    ...
    fn setup_int_single(&mut self) {
        for (name, dir, dend) in [
            ...
            for i in 0..4 {
                let b = self.builder.make_int_wire(&format!("INT.SNG.{}B{}", name, i), "INT.SNG", &[&format!("{}B{}", name, i)]);
                let e = self.builder.make_int_wire(&format!("INT.SNG.{}E{}", name, i), "INT.SNG", &[&format!("{}E{}", name, i)]);
                self.builder.connect_int_wire(e, dir, b);

                if i == 0 && dend == Some(Dir::S) {
                    let e_s = self.builder.make_int_wire(&format!("INT.SNG.{}E_S{}", name, i), "INT.SNG", &[&format!("{}E_S{}", name, i)]);
                    self.builder.connect_int_wire(e_s, Dir::S, e);
                }
                if i == 3 && dend == Some(Dir::N) {
                    let e_n = self.builder.make_int_wire(&format!("INT.SNG.{}E_N{}", name, i), "INT.SNG", &[&format!("{}E_N{}", name, i)]);
                    self.builder.connect_int_wire(e_n, Dir::N, e);
                }
            }
        }
    ...
    fn setup_int_imux(&mut self) {
        ...
        for i in 0..2 {
            self.builder.make_int_wire(&format!("INT.IMUX.CLK{}", i), "INT.IMUX.CLK", &[&format!("CLK{}", i)]);
        }
        for i in 0..2 {
            self.builder.make_int_wire(&format!("INT.IMUX.SR{}", i), "INT.IMUX.SR", &[&format!("SR{}", i)]);
        }
        for i in 0..63 {
            self.builder.make_int_wire(&format!("INT.IMUX.LOGICIN{}", i), "INT.IMUX.LOGICIN", &[
                &format!("LOGICIN_B{}", i),
            ...
            let w = self.builder.make_int_wire(&format!("INT.IMUX.LOGICIN{}.BOUNCE", i), "INT.IMUX.LOGICIN.BOUNCE", &[&format!("LOGICIN{}", i)]);
            if dir == Dir::S {
                let w_s = self.builder.make_int_wire(&format!("INT.IMUX.LOGICIN{}.BOUNCE.S", i), "INT.IMUX.LOGICIN.BOUNCE", &[&format!("LOGICIN_S{}", i)]);
                self.builder.connect_int_wire(w_s, dir, w);
            } else {
                let w_n = self.builder.make_int_wire(&format!("INT.IMUX.LOGICIN{}.BOUNCE.N", i), "INT.IMUX.LOGICIN.BOUNCE", &[&format!("LOGICIN_N{}", i)]);
                self.builder.connect_int_wire(w_n, dir, w);
            }
        }
        self.builder.make_int_wire(&format!("INT.IMUX.LOGICIN{}", 63), "INT.IMUX.LOGICIN", &["FAN_B"]);
    }
