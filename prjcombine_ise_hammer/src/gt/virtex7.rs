...
pub fn add_fuzzers<'a>(session: &mut Session<IseBackend<'a>>, backend: &IseBackend<'a>) {
    for (tile, bits) in [
        ("GTP_COMMON", TileBits::MainAuto),
        ("GTP_COMMON_MID", TileBits::GtpCommonMid),
    ] {
        let Some(ctx) = FuzzCtx::try_new(session, backend, tile, "GTP_COMMON", bits.clone()) else {
            continue;
        };
        fuzz_one!(ctx, "PRESENT", "1", [], [(mode "GTPE2_COMMON")]);
        for &pin in GTP_COMMON_INVPINS {
            fuzz_inv!(ctx, pin, [(mode "GTPE2_COMMON")]);
        }
        for &(attr, vals) in GTP_COMMON_ENUM_ATTRS {
            fuzz_enum!(ctx, attr, vals.iter().copied(), [(mode "GTPE2_COMMON")]);
        }
        for &(attr, width) in GTP_COMMON_BIN_ATTRS {
            fuzz_multi_attr_bin!(ctx, attr, width, [(mode "GTPE2_COMMON")]);
        }
        for &(attr, width) in GTP_COMMON_HEX_ATTRS {
            fuzz_multi_attr_hex!(ctx, attr, width, [(mode "GTPE2_COMMON")]);
        }
    }
    if let Some(ctx) = FuzzCtx::try_new(
        ...
        fuzz_one!(ctx, "PRESENT", "1", [], [(mode "GTXE2_COMMON")]);
        for &pin in GTXH_COMMON_INVPINS {
            fuzz_inv!(ctx, pin, [(mode "GTXE2_COMMON")]);
        }
        for &(attr, vals) in GTXH_COMMON_ENUM_ATTRS {
            fuzz_enum!(ctx, attr, vals.iter().copied(), [(mode "GTXE2_COMMON")]);
        }
        for &(attr, width) in GTX_COMMON_BIN_ATTRS {
            fuzz_multi_attr_bin!(ctx, attr, width, [(mode "GTXE2_COMMON")]);
        }
        for &(attr, width) in GTX_COMMON_HEX_ATTRS {
            fuzz_multi_attr_hex!(ctx, attr, width, [(mode "GTXE2_COMMON")]);
        }
    }
    if let Some(ctx) = FuzzCtx::try_new(
        session,
        ...
        fuzz_one!(ctx, "PRESENT", "1", [], [(mode "GTHE2_COMMON")]);
        for &pin in GTXH_COMMON_INVPINS {
            fuzz_inv!(ctx, pin, [(mode "GTHE2_COMMON")]);
        }
        for &(attr, vals) in GTXH_COMMON_ENUM_ATTRS {
            fuzz_enum!(ctx, attr, vals.iter().copied(), [(mode "GTHE2_COMMON")]);
        }
        for &(attr, width) in GTH_COMMON_BIN_ATTRS {
            fuzz_multi_attr_bin!(ctx, attr, width, [(mode "GTHE2_COMMON")]);
        }
        for &(attr, width) in GTH_COMMON_HEX_ATTRS {
            fuzz_multi_attr_hex!(ctx, attr, width, [(mode "GTHE2_COMMON")]);
        }
    }
    for tile in ["GTX_COMMON", "GTH_COMMON"] {
        let Some(ctx) = FuzzCtx::try_new(session, backend, tile, tile, TileBits::MainAuto) else {
            continue;
        };
        for pin in [
            "GTREFCLK0",
            ...
            fuzz_one!(ctx, "QPLLREFCLKSEL_STATIC", pin, [
                (mutex "QPLLREFCLKSEL_STATIC", pin)
            ], [
                (pip (pin_far pin), (pin pin))
            ]);
        }
        fuzz_one!(ctx, "QPLLREFCLKSEL_MODE", "DYNAMIC", [
            (mutex "QPLLREFCLKSEL_STATIC", "MODE"),
            (pip (pin_far "GTGREFCLK"), (pin "GTGREFCLK"))
        ], [
            (pip (pin_far "GTREFCLK0"), (pin "GTREFCLK0"))
        ]);
        let node = backend.egrid.db.get_node(tile);
        let node_brkh = backend.egrid.db.get_node("BRKH_GTX");
        let bel_brkh = BelId::from_idx(0);
        if backend.egrid.node_index[node].len() > 1 {
            for i in 0..2 {
                fuzz_one!(ctx, format!("MUX.NORTHREFCLK{i}_N"), format!("NORTHREFCLK{i}"), [
                    (mutex format!("MUX.NORTHREFCLK{i}_N"), format!("NORTHREFCLK{i}")),
                    (related TileRelation::Delta(0, 50, node),
                        (nop))
                ], [
                    (related TileRelation::Delta(0, 25, node_brkh),
                        (pip
                            (bel_pin bel_brkh, format!("NORTHREFCLK{i}_D")),
                            (bel_pin bel_brkh, format!("NORTHREFCLK{i}_U"))))
                ]);
                for j in 0..2 {
                    fuzz_one!(ctx, format!("MUX.NORTHREFCLK{i}_N"), format!("REFCLK{j}"), [
                        ...
                        (related TileRelation::Delta(0, 25, node_brkh),
                            (pip
                                (bel_pin bel_brkh, format!("REFCLK{j}_D")),
                                (bel_pin bel_brkh, format!("NORTHREFCLK{i}_U"))))
                    ]);
                }
                fuzz_one!(ctx, format!("MUX.SOUTHREFCLK{i}_S"), format!("SOUTHREFCLK{i}"), [
                    ...
                    (related TileRelation::Delta(0, -25, node_brkh),
                        (pip
                            (bel_pin bel_brkh, format!("SOUTHREFCLK{i}_U")),
                            (bel_pin bel_brkh, format!("SOUTHREFCLK{i}_D"))))
                ]);
                for j in 0..2 {
                    fuzz_one!(ctx, format!("MUX.SOUTHREFCLK{i}_S"), format!("REFCLK{j}"), [
                        ...
                        (related TileRelation::Delta(0, -25, node_brkh),
                            (pip
                                (bel_pin bel_brkh, format!("REFCLK{j}_U")),
                                (bel_pin bel_brkh, format!("SOUTHREFCLK{i}_D"))))
                    ]);
                }
            }
    ...
    for (tile, bits) in [
        ("GTP_COMMON", TileBits::MainAuto),
        ("GTP_COMMON_MID", TileBits::GtpCommonMid),
        ("GTX_COMMON", TileBits::MainAuto),
        ("GTH_COMMON", TileBits::MainAuto),
    ] {
        for i in 0..2 {
            let Some(ctx) =
                FuzzCtx::try_new(session, backend, tile, format!("IBUFDS{i}"), bits.clone())
            else {
                continue;
            };
            fuzz_one!(ctx, "PRESENT", "1", [], [(mode "IBUFDS_GTE2")]);
            fuzz_inv!(ctx, "CLKTESTSIG", [(mode "IBUFDS_GTE2")]);
            fuzz_enum!(ctx, "CLKCM_CFG", ["FALSE", "TRUE"], [(mode "IBUFDS_GTE2")]);
            fuzz_enum!(ctx, "CLKRCV_TRST", ["FALSE", "TRUE"], [(mode "IBUFDS_GTE2")]);
            fuzz_multi_attr_bin!(ctx, "CLKSWING_CFG", 2, [
                (mode "IBUFDS_GTE2"),
                (tile_mutex "CLKSWING_CFG", format!("IBUFDS{i}"))
            ]);
            for pin in ["O", "ODIV2"] {
                fuzz_one!(ctx, "MUX.MGTCLKOUT", pin, [
                    (mode "IBUFDS_GTE2"),
                    (mutex "MUX.MGTCLKOUT", pin)
                ], [
                    (pip (pin pin), (pin "MGTCLKOUT"))
                ]);
            }
            fuzz_one!(ctx, "MUX.MGTCLKOUT", "CLKTESTSIG", [
                (mode "IBUFDS_GTE2"),
    ...
    if let Some(ctx) = FuzzCtx::try_new(
        ...
        for i in 0..14 {
            ...
            for pin in [
                ...
                fuzz_one!(ctx, format!("MUX.HOUT{i}"), pin, [
                    (tile_mutex "HIN", "USE"),
                    (special TileKV::TouchHout(i)),
                    (special TileKV::TouchHout(oi)),
                    (mutex format!("MUX.HOUT{i}"), pin),
                    (mutex format!("MUX.HOUT{oi}"), pin),
                    (pip (pin format!("{pin}_BUF")), (pin format!("HOUT{oi}")))
                ], [
                    (pip (pin format!("{pin}_BUF")), (pin format!("HOUT{i}")))
                ]);
                if i == 0 {
                    fuzz_one!(ctx, format!("MUX.HOUT{i}"), format!("{pin}.EXCL"), [
                        (tile_mutex "HIN", "TEST"),
                        (special TileKV::TouchHout(i)),
                        (mutex format!("MUX.HOUT{i}"), pin)
                    ], [
    ...
    for (tile, bits) in [
        ("GTP_CHANNEL", TileBits::MainAuto),
        ("GTP_CHANNEL_MID", TileBits::GtpChannelMid),
    ] {
        let Some(ctx) = FuzzCtx::try_new(session, backend, tile, "GTP_CHANNEL", bits) else {
            continue;
        };
        fuzz_one!(ctx, "PRESENT", "1", [], [(mode "GTPE2_CHANNEL")]);
        for &pin in GTP_CHANNEL_INVPINS {
            fuzz_inv!(ctx, pin, [(mode "GTPE2_CHANNEL")]);
        }
        for &attr in GTP_CHANNEL_BOOL_ATTRS {
            fuzz_enum!(ctx, attr, ["FALSE", "TRUE"], [(mode "GTPE2_CHANNEL")]);
        }
        for &(attr, vals) in GTP_CHANNEL_ENUM_ATTRS {
            fuzz_enum!(ctx, attr, vals.iter().copied(), [(mode "GTPE2_CHANNEL")]);
        }
        for &(attr, ref vals, delta) in GTP_CHANNEL_ENUM_INT_ATTRS {
            let vals = Vec::from_iter(vals.clone().map(|i| (i + delta).to_string()));
            fuzz_enum!(ctx, attr, vals.iter(), [(mode "GTPE2_CHANNEL")]);
        }
        for &(attr, width) in GTP_CHANNEL_DEC_ATTRS {
            fuzz_multi_attr_dec!(ctx, attr, width, [(mode "GTPE2_CHANNEL")]);
        }
        for &(attr, width) in GTP_CHANNEL_BIN_ATTRS {
            fuzz_multi_attr_bin!(ctx, attr, width, [(mode "GTPE2_CHANNEL")]);
        }
        for &(attr, width) in GTP_CHANNEL_HEX_ATTRS {
            fuzz_multi_attr_hex!(ctx, attr, width, [(mode "GTPE2_CHANNEL")]);
        }
    }
    if let Some(ctx) = FuzzCtx::try_new(
        session,
        ...
        fuzz_one!(ctx, "PRESENT", "1", [], [(mode "GTXE2_CHANNEL")]);
        for &pin in GTX_CHANNEL_INVPINS {
            fuzz_inv!(ctx, pin, [(mode "GTXE2_CHANNEL")]);
        }
        for &attr in GTX_CHANNEL_BOOL_ATTRS {
            fuzz_enum!(ctx, attr, ["FALSE", "TRUE"], [(mode "GTXE2_CHANNEL")]);
        }
        for &(attr, vals) in GTX_CHANNEL_ENUM_ATTRS {
            fuzz_enum!(ctx, attr, vals.iter().copied(), [(mode "GTXE2_CHANNEL")]);
        }
        for &(attr, ref vals, delta) in GTX_CHANNEL_ENUM_INT_ATTRS {
            let vals = Vec::from_iter(vals.clone().map(|i| (i + delta).to_string()));
            fuzz_enum!(ctx, attr, vals.iter(), [(mode "GTXE2_CHANNEL")]);
        }
        for &(attr, width) in GTX_CHANNEL_DEC_ATTRS {
            fuzz_multi_attr_dec!(ctx, attr, width, [(mode "GTXE2_CHANNEL")]);
        }
        for &(attr, width) in GTX_CHANNEL_BIN_ATTRS {
            fuzz_multi_attr_bin!(ctx, attr, width, [(mode "GTXE2_CHANNEL")]);
        }
        for &(attr, width) in GTX_CHANNEL_HEX_ATTRS {
            fuzz_multi_attr_hex!(ctx, attr, width, [(mode "GTXE2_CHANNEL")]);
        }
    }
    if let Some(ctx) = FuzzCtx::try_new(
        session,
        ...
        fuzz_one!(ctx, "PRESENT", "1", [], [(mode "GTHE2_CHANNEL")]);
        for &pin in GTH_CHANNEL_INVPINS {
            fuzz_inv!(ctx, pin, [(mode "GTHE2_CHANNEL")]);
        }
        for &attr in GTH_CHANNEL_BOOL_ATTRS {
            fuzz_enum!(ctx, attr, ["FALSE", "TRUE"], [(mode "GTHE2_CHANNEL")]);
        }
        for &(attr, vals) in GTH_CHANNEL_ENUM_ATTRS {
            fuzz_enum!(ctx, attr, vals.iter().copied(), [(mode "GTHE2_CHANNEL")]);
        }
        for &(attr, ref vals, delta) in GTH_CHANNEL_ENUM_INT_ATTRS {
            let vals = Vec::from_iter(vals.clone().map(|i| (i + delta).to_string()));
            fuzz_enum!(ctx, attr, vals.iter(), [(mode "GTHE2_CHANNEL")]);
        }
        for &(attr, width) in GTH_CHANNEL_DEC_ATTRS {
            fuzz_multi_attr_dec!(ctx, attr, width, [(mode "GTHE2_CHANNEL")]);
        }
        for &(attr, width) in GTH_CHANNEL_BIN_ATTRS {
            fuzz_multi_attr_bin!(ctx, attr, width, [(mode "GTHE2_CHANNEL")]);
        }
        for &(attr, width) in GTH_CHANNEL_HEX_ATTRS {
            fuzz_multi_attr_hex!(ctx, attr, width, [(mode "GTHE2_CHANNEL")]);
        }
    }
    for tile in ["GTX_CHANNEL", "GTH_CHANNEL"] {
        ...
        for pin in [
            ...
            fuzz_one!(ctx, "CPLLREFCLKSEL_STATIC", pin, [
                (mutex "CPLLREFCLKSEL_STATIC", pin)
            ], [
                (pip (pin_far pin), (pin pin))
            ]);
        }
        fuzz_one!(ctx, "CPLLREFCLKSEL_MODE", "DYNAMIC", [
