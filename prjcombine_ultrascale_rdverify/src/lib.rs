...
use prjcombine_ultrascale::{
    ColumnKindLeft, DisabledPart, ExpandedDevice, GridKind, HardRowKind, HdioIobId, HpioIobId,
};
...
fn verify_bufce_row(edev: &ExpandedDevice, vrf: &mut Verifier, bel: &BelContext<'_>) {
    ...
    let kind = if grid.kind == GridKind::UltrascalePlus {
        "BUFCE_ROW_FSR"
    } else {
        "BUFCE_ROW"
    };
...
fn verify_bitslice_tx(edev: &ExpandedDevice, vrf: &mut Verifier, bel: &BelContext<'_>) {
    ...
    let feed_pins = if grid.kind == GridKind::Ultrascale {
        [
            (
                "REGRST_B",
...
fn verify_rclk_xiphy(_edev: &ExpandedDevice, vrf: &mut Verifier, bel: &BelContext<'_>) {
    ...
    for i in 0..6 {
        for bt in ['B', 'T'] {
            ...
            vrf.claim_pip(bel.crd(), bel.wire(&pin), obel_vcc.wire("VCC"));
            for j in 0..24 {
                vrf.claim_pip(bel.crd(), bel.wire(&pin), bel.wire(&format!("HDISTR{j}_R")));
            }
        }
    }
    for i in 0..24 {
...
fn verify_hpiob(edev: &ExpandedDevice, vrf: &mut Verifier, bel: &BelContext<'_>) {
    ...
    if grid.kind == GridKind::Ultrascale || !matches!(idx, 12 | 25) {
        pins.push(("PAD_RES", SitePinDir::Out));
    }

    let mut dummies = vec!["TSDI"];
    if matches!(idx, 12 | 25) {
        dummies.push("IO");
        if grid.kind == GridKind::UltrascalePlus {
    ...
    let obel_bs = if grid.kind == GridKind::Ultrascale {
        let srow = grid.row_rclk(bel.row);
        vrf.find_bel(
            bel.die,
            (bel.col, srow),
...
fn verify_gt_channel(edev: &ExpandedDevice, vrf: &mut Verifier, bel: &BelContext<'_>) {
    ...
    let cross_qdpll = grid.kind == GridKind::Ultrascale && bel.key.starts_with("GTH");
    for (pin, opin) in [
...
fn verify_rclk_gt(_edev: &ExpandedDevice, vrf: &mut Verifier, bel: &BelContext<'_>) {
    ...
    for i in 0..24 {
        ...
        // XXX source/claim HDISTR, HROUTE
    }
}
