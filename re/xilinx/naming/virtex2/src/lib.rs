...
impl Namer<'_> {
    ...
    fn fill_io_t(&mut self) {
        ...
        for (col, &cd) in &self.chip.columns {
            ...
            for &i in iobs {
                if clks.contains(&i) {
                    let (slot, name) = match i {
                        0 => (bels::CLK_P, "CLKPPAD1"),
                        1 => (bels::CLK_N, "CLKNPAD1"),
                        _ => unreachable!(),
                    };
                    nnode.add_bel(slot, name.into());
                    self.ctr_pad += 1;
                } else if pads.contains(&i) {
                    nnode.add_bel(bels::IO[i], format!("PAD{idx}", idx = self.ctr_pad));
                    self.ctr_pad += 1;
                } else if ipads.contains(&i) {
                    nnode.add_bel(bels::IO[i], format!("IPAD{idx}", idx = self.ctr_pad));
                    self.ctr_pad += 1;
                } else {
                    nnode.add_bel(bels::IO[i], format!("NOPAD{idx}", idx = self.ctr_nopad));
                    self.ctr_nopad += 1;
                }
            }
        }
    ...
    fn fill_io_b(&mut self) {
        ...
        for (col, &cd) in self.chip.columns.iter().rev() {
            ...
            for &i in iobs {
                if clks.contains(&i) {
                    let (slot, name) = match i {
                        2 => (bels::CLK_P, "CLKPPAD2"),
                        3 => (bels::CLK_N, "CLKNPAD2"),
                        _ => unreachable!(),
                    };
                    nnode.add_bel(slot, name.into());
                    self.ctr_pad += 1;
