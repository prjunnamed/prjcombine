...
pub fn add_fuzzers<'a>(session: &mut Session<IseBackend<'a>>, backend: &IseBackend<'a>) {
    ...
    for &pin in GTP_INVPINS {
        fuzz_inv!(ctx, pin, [(mode "GTPA1_DUAL")]);
    }
    for &attr in GTP_BOOL_ATTRS {
        fuzz_enum!(ctx, format!("{attr}_0"), ["FALSE", "TRUE"], [(mode "GTPA1_DUAL")]);
        fuzz_enum!(ctx, format!("{attr}_1"), ["FALSE", "TRUE"], [(mode "GTPA1_DUAL")]);
    }
    for &(attr, vals) in GTP_ENUM_ATTRS {
        fuzz_enum!(ctx, format!("{attr}_0"), vals.iter().copied(), [(mode "GTPA1_DUAL")]);
        fuzz_enum!(ctx, format!("{attr}_1"), vals.iter().copied(), [(mode "GTPA1_DUAL")]);
    }
    for &(attr, ref vals) in GTP_ENUM_INT_ATTRS {
        let vals = Vec::from_iter(vals.clone().map(|i| i.to_string()));
        fuzz_enum!(ctx, format!("{attr}_0"), vals.iter(), [(mode "GTPA1_DUAL")]);
        fuzz_enum!(ctx, format!("{attr}_1"), vals.iter(), [(mode "GTPA1_DUAL")]);
    }
    for &(attr, width) in GTP_DEC_ATTRS {
        fuzz_multi_attr_dec!(ctx, format!("{attr}_0"), width, [(mode "GTPA1_DUAL")]);
        fuzz_multi_attr_dec!(ctx, format!("{attr}_1"), width, [(mode "GTPA1_DUAL")]);
    }
    for &(attr, width) in GTP_BIN_ATTRS {
        fuzz_multi_attr_bin!(ctx, format!("{attr}_0"), width, [(mode "GTPA1_DUAL")]);
        fuzz_multi_attr_bin!(ctx, format!("{attr}_1"), width, [(mode "GTPA1_DUAL")]);
    }
    for &(attr, width) in GTP_HEX_ATTRS {
        fuzz_multi_attr_hex!(ctx, format!("{attr}_0"), width, [(mode "GTPA1_DUAL")]);
        fuzz_multi_attr_hex!(ctx, format!("{attr}_1"), width, [(mode "GTPA1_DUAL")]);
    }

    fuzz_enum!(ctx, "CLK_OUT_GTP_SEL_0", ["TXOUTCLK0", "REFCLKPLL0"], [(mode "GTPA1_DUAL")]);
    fuzz_enum!(ctx, "CLK_OUT_GTP_SEL_1", ["TXOUTCLK1", "REFCLKPLL1"], [(mode "GTPA1_DUAL")]);

    fuzz_multi_attr_hex!(ctx, "PMA_COM_CFG_EAST", 36, [(mode "GTPA1_DUAL")]);
    fuzz_multi_attr_hex!(ctx, "PMA_COM_CFG_WEST", 36, [(mode "GTPA1_DUAL")]);

    for i in 0..2 {
        for pin in ["PLLCLK0", "PLLCLK1", "CLKINEAST", "CLKINWEST"] {
            fuzz_one!(ctx, format!("REFSELPLL{i}"), pin, [
                (mutex format!("REFSELPLL{i}"), pin)
            ], [
                (pip (pin pin), (pin format!("{pin}{i}")))
            ]);
        }
        for (pin, obel) in [("CLK0", BelId::from_idx(12)), ("CLK1", BelId::from_idx(13))] {
            fuzz_one!(ctx, format!("REFSELPLL{i}"), pin, [
                (mutex format!("REFSELPLL{i}"), pin)
            ], [
                (pip (bel_pin obel, "O"), (pin format!("{pin}{i}")))
            ]);
        }
