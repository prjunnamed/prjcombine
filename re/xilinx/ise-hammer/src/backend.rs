...
use prjcombine_interconnect::db::{BelInputId, TileWireCoord};
...
impl From<TileWireCoord> for Value<'_> {
    fn from(value: TileWireCoord) -> Self {
        Self::TileWire(value)
    }
}
...
impl<'a> Backend for IseBackend<'a> {
    ...
    fn bitgen(&self, kv: &HashMap<Key, Value>) -> Bitstream {
        ...
        if let Bond::Xc2000(bond) = bond {
            ...
            for io in endev.chip.get_bonded_ios() {
                let name = endev.get_io_name(io);
            ...
            match k {
                ...
                Key::Pip(tile, wa, wb) => match v {
                    ...
                    Value::Bool(true) => single_pips.push(NetPip {
                        ...
                        dir: prjcombine_re_xilinx_xdl::PipDirection::UniBuf,
                    }),
