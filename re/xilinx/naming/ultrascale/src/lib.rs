    ...
    pub fn get_io_name(&self, io: IoCoord) -> &str {
        match io {
            ...
            IoCoord::Hdio(hdio) => {
                ...
                self.ngrid
                    .get_bel_name((hdio.die, (hdio.col, row), bels::HDIOB[idx]))
                    .unwrap()
            }
            IoCoord::HdioLc(hdio) => {
                let chip = self.edev.chips[hdio.die];
    ...
    pub fn get_gts(&self) -> Vec<Gt<'_>> {
        ...
        for &crd in &self.edev.gt {
            ...
            let (name_common, name_channel) = match gt_info.kind {
                IoRowKind::Gth => (
                    self.ngrid
                        .get_bel_name((crd.die, (crd.col, row), bels::GTH_COMMON))
                        .unwrap(),
                    vec![
                        self.ngrid
                            .get_bel_name((crd.die, (crd.col, row), bels::GTH_CHANNEL0))
                            .unwrap(),
                        self.ngrid
                            .get_bel_name((crd.die, (crd.col, row), bels::GTH_CHANNEL1))
                            .unwrap(),
                        self.ngrid
                            .get_bel_name((crd.die, (crd.col, row), bels::GTH_CHANNEL2))
                            .unwrap(),
                        self.ngrid
                            .get_bel_name((crd.die, (crd.col, row), bels::GTH_CHANNEL3))
                            .unwrap(),
                    ],
                ),
                IoRowKind::Gty => (
                    self.ngrid
                        .get_bel_name((crd.die, (crd.col, row), bels::GTY_COMMON))
                        .unwrap(),
                    vec![
                        self.ngrid
                            .get_bel_name((crd.die, (crd.col, row), bels::GTY_CHANNEL0))
                            .unwrap(),
                        self.ngrid
                            .get_bel_name((crd.die, (crd.col, row), bels::GTY_CHANNEL1))
                            .unwrap(),
                        self.ngrid
                            .get_bel_name((crd.die, (crd.col, row), bels::GTY_CHANNEL2))
                            .unwrap(),
                        self.ngrid
                            .get_bel_name((crd.die, (crd.col, row), bels::GTY_CHANNEL3))
                            .unwrap(),
                    ],
                ),
                IoRowKind::Gtm => (
                    self.ngrid
                        .get_bel_name((crd.die, (crd.col, row), bels::GTM_REFCLK))
                        .unwrap(),
                    vec![
                        self.ngrid
                            .get_bel_name((crd.die, (crd.col, row), bels::GTM_DUAL))
                            .unwrap(),
                    ],
                ),
                IoRowKind::Gtf => (
                    self.ngrid
                        .get_bel_name((crd.die, (crd.col, row), bels::GTF_COMMON))
                        .unwrap(),
                    vec![
                        self.ngrid
                            .get_bel_name((crd.die, (crd.col, row), bels::GTF_CHANNEL0))
                            .unwrap(),
                        self.ngrid
                            .get_bel_name((crd.die, (crd.col, row), bels::GTF_CHANNEL1))
                            .unwrap(),
                        self.ngrid
                            .get_bel_name((crd.die, (crd.col, row), bels::GTF_CHANNEL2))
                            .unwrap(),
                        self.ngrid
                            .get_bel_name((crd.die, (crd.col, row), bels::GTF_CHANNEL3))
                            .unwrap(),
                    ],
                ),
                IoRowKind::HsAdc => (
                    self.ngrid
                        .get_bel_name((crd.die, (crd.col, row), bels::HSADC))
                        .unwrap(),
                    vec![],
                ),
                IoRowKind::HsDac => (
                    self.ngrid
                        .get_bel_name((crd.die, (crd.col, row), bels::HSDAC))
                        .unwrap(),
                    vec![],
                ),
                IoRowKind::RfAdc => (
                    self.ngrid
                        .get_bel_name((crd.die, (crd.col, row), bels::RFADC))
                        .unwrap(),
                    vec![],
                ),
                IoRowKind::RfDac => (
                    self.ngrid
                        .get_bel_name((crd.die, (crd.col, row), bels::RFDAC))
                        .unwrap(),
                    vec![],
                ),
                _ => unreachable!(),
...
pub fn name_device<'a>(
    ...
    for die in egrid.dies() {
        ...
        let hdio_cfg_only: Vec<_> = chip
            ...
            .collect();
        let has_mcap = chip.cols_hard.iter().any(|hcol| {
        ...
        for col in die.cols() {
            for row in die.rows() {
                ...
                for (layer, node) in &die[(col, row)].tiles {
                    ...
                    match &kind[..] {
                        "INT" => {
                            ngrid.name_tile(nloc, "INT", [format!("INT_X{x}Y{y}")]);
                        }
                        ...
                        "RCLK_INT" => {
                            ...
                            let nnode = ngrid.name_tile(nloc, "RCLK_INT", [name]);
                        ...
                        "CLEL" => {
                            ...
                            let nnode = ngrid.name_tile(nloc, "CLEL", [format!("{tkn}_X{x}Y{y}")]);
                            if !(row.to_idx() % 60 == 59
                                && edev
                                    .disabled
                        ...
                        "CLEM" => {
                            ...
                            let nnode = ngrid.name_tile(nloc, "CLEM", [format!("{tk}_X{x}Y{y}")]);
                            if !(row.to_idx() % 60 == 59
                                && edev
                                    .disabled
                                    .contains(&DisabledPart::TopRow(die.die, chip.row_to_reg(row))))
                        ...
                        "LAGUNA" => {
                            ...
                            let nnode = ngrid.name_tile(nloc, "LAGUNA", [format!("{tk}_X{x}Y{y}")]);
                        ...
                        "BRAM" => {
                            let nnode = ngrid.name_tile(nloc, "BRAM", [format!("BRAM_X{x}Y{y}")]);
                        ...
                        "DSP" => {
                            let nnode = ngrid.name_tile(nloc, "DSP", [format!("DSP_X{x}Y{y}")]);
                        ...
                        "URAM" => {
                            ...
                            let nnode = ngrid.name_tile(nloc, "URAM", [format!("{tk}_X{x}Y{y}")]);
                        ...
                        "FE" => {
                            ...
                            let nnode = ngrid.name_tile(nloc, "FE", [name]);
                        ...
                        "HDIO_S" | "HDIO_N" => {
                            ...
                            nnode
                                .add_bel(bels::HDLOGIC_CSSD[0], format!("HDLOGIC_CSSD_X{sx}Y{sy}"));
                            if kind == "HDIO_S" {
                                nnode.add_bel(
                                    bels::HDIO_VREF0,
                        ...
                        "CMT" | "CMT_HBM" => {
                            ...
                            for i in 0..2 {
                                nnode.add_bel(
                                    bels::PLL[i],
                                    format!("PLL_X{cmtx}Y{y}", y = cmty * 2 + i),
                                );
                            }
                        ...
                        "XIPHY" => {
                            ...
                            let nnode = ngrid.name_tile(nloc, kind, [format!("{tk}_X{x}Y{y}")]);
                        ...
                        "GTH" | "GTY" | "GTF" | "GTM" | "HSADC" | "HSDAC" | "RFADC" | "RFDAC" => {
                            let (tk, naming, gtk_grid) =
                                match (chip.kind, &kind[..], chip.col_side(col)) {
                                    ...
                                    (ChipKind::Ultrascale, "GTH", DirH::E) => {
                                    ...
                                    (ChipKind::UltrascalePlus, "GTY", DirH::W) => {
                                        ("GTY_L", "GTY_L", &gty_grid)
                                    }
                                    (ChipKind::UltrascalePlus, "GTY", DirH::E) => {
                        ...
                        "PS" => {
                            let nnode = ngrid.name_tile(nloc, "PS", [format!("PSS_ALTO_X0Y{y}")]);
                            nnode.add_bel(bels::PS, "PS8_X0Y0".to_string());
                        }
                        "VCU" => {
                            let nnode =
                                ngrid.name_tile(nloc, "VCU", [format!("VCU_VCU_FT_X0Y{y}")]);
                        ...
                        "BLI" => {
                            let nnode =
                                ngrid.name_tile(nloc, "BLI", [format!("BLI_BLI_FT_X{x}Y{y}")]);
                            let dx = dsp_grid.xlut[col];
                            nnode.add_bel(
                        ...
                        "RCLK_HROUTE_SPLITTER.HARD" => {
                            ...
                            ngrid.name_tile(nloc, "RCLK_HROUTE_SPLITTER", [name]);
                        }
                        "HBM_ABUS_SWITCH" => {
                            let nnode = ngrid.name_tile(
                                nloc,
