...
fn verify_clk_mux(edev: &ExpandedDevice, vrf: &mut Verifier, bel: &BelContext<'_>) {
    ...
    for i in 0..32 {
        ...
        for j in 0..5 {
            if edev.col_lgt.is_some() || bel.key != "CLK_MGT" {
                vrf.claim_pip(
                    bel.crd(),
                    bel.wire(&format!("MUXBUS_O{i}")),
...
fn verify_iob(vrf: &mut Verifier, bel: &BelContext<'_>) {
    let kind = if bel.key == "IOB0" { "IOBM" } else { "IOBS" };
    let pins = [
        ("I", SitePinDir::Out),
        ("O", SitePinDir::In),
        ("T", SitePinDir::In),
...
fn verify_mgt_conn(
    ...
    } else if !is_l && edev.col_rio.is_some() {
        let obel = vrf
            .find_bel(bel.die, (edev.col_rio.unwrap(), bel.row), "RCLK")
            .unwrap();
        for i in 0..5 {
            vrf.verify_node(&[
                bel.fwire(&format!("{pref}{i}")),
