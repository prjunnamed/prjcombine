...
pub fn extract_ff(
    ...
    extract_delay(sh0.setup, tgt, tname_setup_d_clk);
    extract_delay(sh0.hold, tgt, tname_hold_d_clk);
    let sh1 = &cell.setuphold[1];
    assert_eq!(sh1.edge_d, Edge::Negedge("I".into()));
    assert_eq!(sh1.edge_c, Edge::Posedge("CLK".into()));
    extract_delay(sh1.setup, tgt, tname_setup_d_clk);
    extract_delay(sh1.hold, tgt, tname_hold_d_clk);
    ...
    if let Some(tname) = tname_setup_ce_clk {
        extract_delay(sh2.setup, tgt, tname);
    }
    if let Some(tname) = tname_hold_ce_clk {
        extract_delay(sh2.hold, tgt, tname);
    }
