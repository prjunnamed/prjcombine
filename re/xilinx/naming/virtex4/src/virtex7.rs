    ...
    for (tcrd, tile) in egrid.tiles() {
        ...
        match &kind[..] {
            ...
            "PMVBRAM_NC" => {
                ...
                let hy = raw_grid.ylut[die][row] - 1;
                let nnode = ngrid.name_tile(tcrd, "PMVBRAM_NC", [format!("HCLK_BRAM_X{hx}Y{hy}")]);
                let bx = pmvbram_grid.xlut[col];
                let by = pmvbram_grid.ylut[die][row];
            ...
            "IO_HP_BOT" | "IO_HP_TOP" | "IO_HP_PAIR" | "IO_HR_BOT" | "IO_HR_TOP" | "IO_HR_PAIR" => {
                ...
                let nnode = ngrid.name_tile(tcrd, &ioi_tk, [name, name_iob]);
            ...
            "CLK_HROW" => {
                ...
                for i in 0..12 {
                    nnode.add_bel(bels::BUFHCE_W[i], format!("BUFHCE_X0Y{y}", y = bufh_y + i));
                }
                for i in 0..12 {
                    nnode.add_bel(bels::BUFHCE_E[i], format!("BUFHCE_X1Y{y}", y = bufh_y + i));
                }
                nnode.add_bel(
            ...
            "CLK_BUFG" => {
                ...
                for i in 0..16 {
                    nnode.add_bel(bels::BUFGCTRL[i], format!("BUFGCTRL_X0Y{y}", y = bg_y + i));
                }
            }
            "CLK_PMV" => {
                let nnode = ngrid.name_tile(
                ...
                nnode.add_bel(bels::PMV0, format!("PMV_X0Y{y}", y = die.to_idx() * 3));
            }
            "CLK_PMVIOB" => {
                let nnode = ngrid.name_tile(
                    tcrd,
            ...
            "CLK_PMV2" => {
                ...
                nnode.add_bel(bels::PMV2, format!("PMV_X0Y{y}", y = die.to_idx() * 3 + 2));
            }
            "CLK_MTBF2" => {
                let nnode = ngrid.name_tile(
                    tcrd,
                ...
                nnode.add_bel(bels::MTBF2, format!("MTBF2_X0Y{y}", y = die.to_idx()));
            }
            "CFG" => {
            ...
            "SYSMON" => {
                ...
                nnode.add_bel(bels::SYSMON, format!("XADC_X0Y{di}", di = die.to_idx()));
            }
            "PS" => {
                ...
                for i in 0..54 {
                    nnode.add_bel(bels::IOPAD_MIO[i], format!("IOPAD_X1Y{ii}", ii = i + 77));
                }
            ...
            "GTP_CHANNEL" | "GTP_CHANNEL_MID" | "GTX_CHANNEL" | "GTH_CHANNEL" => {
                ...
                let nnode = ngrid.name_tile(tcrd, &naming, [format!("{naming}_X{rx}Y{ry}")]);
            ...
            "GTP_COMMON" | "GTP_COMMON_MID" | "GTX_COMMON" | "GTH_COMMON" => {
                ...
                nnode.add_bel(bels::BUFDS0, format!("IBUFDS_GTE2_X{gtx}Y{y}", y = gty * 2));
                nnode.add_bel(
                    bels::BUFDS1,
                    format!("IBUFDS_GTE2_X{gtx}Y{y}", y = gty * 2 + 1),
                );
                nnode.add_bel(bels::IPAD_CLKP0, format!("IPAD_X{ipx}Y{y}", y = ipy - 4));
                nnode.add_bel(bels::IPAD_CLKN0, format!("IPAD_X{ipx}Y{y}", y = ipy - 3));
                nnode.add_bel(bels::IPAD_CLKP1, format!("IPAD_X{ipx}Y{y}", y = ipy - 2));
                nnode.add_bel(bels::IPAD_CLKN1, format!("IPAD_X{ipx}Y{y}", y = ipy - 1));
            }
            "BRKH_GTX" => {
                let gtcol = chip.cols_gt.iter().find(|gtcol| gtcol.col == col).unwrap();
                ngrid.name_tile(
