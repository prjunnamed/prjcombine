...
use super::cfg::{GeomBuilderConfig, IntDoubleBufInfo, IntTermInfo, IntTileInfo, TieSiteInfo};
...
impl Virtex5GeomMaker {
    ...
    fn setup_int_ll(&mut self) {
        ...
        for i in 0..9 {
            ...
            self.builder
                .connect_int_wire(lh[18 - i], Dir::E, lh[18 - i - 1]);
        }
    ...
    fn setup_int_pent(&mut self) {
        ...
        .copied()
        {
            for i in 0..3 {
                ...
                let mid = self.builder.make_int_wire(
                    &format!("INT.{}MID{}", name, i),
                    "INT.PENT",
                    &[&format!("{}MID{}", name, i)],
                );
                ...
                if dmid == Some(Dir::S) && i == 0 {
                    let mid_fake = self.builder.make_int_wire(
                        &format!("INT.{}MID_FAKE{}", name, i),
                        "INT.PENT",
                        &[&format!("{}MID_FAKE{}", name, i)],
                    );
                    let mid_s = self.builder.make_int_wire(
                        &format!("INT.{}MID_S{}", name, i),
                        "INT.PENT",
                        &[&format!("{}MID_S{}", name, i)],
                    );
                ...
                if dmid == Some(Dir::N) && i == 2 {
                    ...
                    let mid_n = self.builder.make_int_wire(
                        &format!("INT.{}MID_N{}", name, i),
                        "INT.PENT",
                        &[&format!("{}MID_N{}", name, i)],
                    );
    ...
    fn setup_int_imux(&mut self) {
        ...
        for i in 0..4 {
            ...
            self.builder.make_int_wire(
                &format!("INT.IMUX.CTRL{}.SITE", i),
                "INT.IMUX.CTRL_SITE",
                &[&format!("CTRL_B{}", i)],
            );
            let w = self.builder.make_int_wire(
                &format!("INT.IMUX.CTRL{}.BOUNCE", i),
                "INT.IMUX.CTRL_BOUNCE",
                &[&format!("CTRL_BOUNCE{}", i)],
            );
            if i == 0 {
                let w_s = self.builder.make_int_wire(
                    &format!("INT.IMUX.CTRL{}.BOUNCE.S", i),
                    "INT.IMUX.CTRL_BOUNCE",
                    &[&format!("CTRL_BOUNCE_S{}", i)],
                );
                self.builder.connect_int_wire(w_s, Dir::S, w);
            }
            if i == 3 {
                let w_n = self.builder.make_int_wire(
                    &format!("INT.IMUX.CTRL{}.BOUNCE.N", i),
                    "INT.IMUX.CTRL_BOUNCE",
                    &[&format!("CTRL_BOUNCE_N{}", i)],
                );
        ...
        for i in 0..8 {
            ...
            if i == 0 || i == 4 {
                let w_s = self.builder.make_int_wire(
                    &format!("INT.IMUX.BYP{}.BOUNCE.S", i),
                    "INT.IMUX.BYP_BOUNCE",
                    &[&format!("BYP_BOUNCE_S{}", i)],
                );
                self.builder.connect_int_wire(w_s, Dir::S, w);
            }
            if i == 3 || i == 7 {
        ...
        for i in 0..8 {
            ...
            if i == 0 {
                let w_s = self.builder.make_int_wire(
                    ...
                    "INT.IMUX.FAN_BOUNCE",
                    &[&format!("FAN_BOUNCE_S{}", i)],
                );
                self.builder.connect_int_wire(w_s, Dir::S, w);
            }
            if i == 7 {
                let w_n = self.builder.make_int_wire(
                    &format!("INT.IMUX.FAN{}.BOUNCE.N", i),
                    "INT.IMUX.FAN_BOUNCE",
                    &[&format!("FAN_BOUNCE_N{}", i)],
                );
                self.builder.connect_int_wire(w_n, Dir::N, w);
            }
        }
    ...
    fn setup_int_out(&mut self) {
        for i in 0..24 {
            ...
            if i == 15 || i == 17 {
                let w_n2 = self.builder.make_int_wire(
                    &format!("INT.OUT{}.N2", i),
                    "INT.OUT",
                    &[&format!("LOGIC_OUTS_N{}", i)],
                );
                let w_n5 = self.builder.make_int_wire(
                    &format!("INT.OUT{}.N5", i),
                    "INT.OUT",
            ...
            if i == 12 || i == 18 {
                let w_s2 = self.builder.make_int_wire(
                    &format!("INT.OUT{}.S2", i),
                    "INT.OUT",
                    &[&format!("LOGIC_OUTS_S{}", i)],
                );
                let w_s5 = self.builder.make_int_wire(
                    &format!("INT.OUT{}.S5", i),
                    "INT.OUT",
