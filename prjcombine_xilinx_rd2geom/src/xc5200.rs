...
use prjcombine_xilinx_geom::{self as geom, int, int::Dir, Bond, BondPin};
...
fn make_int_db(rd: &Part) -> int::IntDb {
    ...
    for i in 0..8 {
        let w = builder.wire(
            format!("LONG.H{i}"),
            int::WireKind::MultiBranch(Dir::W),
            &[
    ...
    for i in 0..8 {
        let w = builder.wire(
            format!("LONG.V{i}"),
            int::WireKind::MultiBranch(Dir::S),
            &[
