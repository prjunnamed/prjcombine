...
struct IntMaker<'a> {
    ...
    term_wires_e: EntityPartVec<WireId, TermInfo>,
    term_wires_lw: EntityPartVec<WireId, TermInfo>,
    term_wires_le: EntityPartVec<WireId, TermInfo>,
...
impl IntMaker<'_> {
        ...
        for (dir, name, length, fts, ftn) in [
            ...
            for i in 0..8 {
                ...
                match (length, dir) {
                    (1, Dir::W) => {
                        self.sng_fixup_map
                            .insert((NodeTileId::from_idx(1), w0), (NodeTileId::from_idx(0), w));
                    }
                    (1, Dir::E) => {
                        self.sng_fixup_map
                            .insert((NodeTileId::from_idx(0), w0), (NodeTileId::from_idx(1), w));
                    }
                    _ => (),
                }
                if i == 0 && fts {
    ...
    fn extract_sn_term(&mut self, dir: Dir, int_xy: Coord) {
        let pass_rev =
            &self.builder.db.terms[self.builder.db.get_term(&format!("MAIN.{rd}", rd = !dir))];
        let naming = &self.builder.ndb.node_namings[self.builder.ndb.get_node_naming("INT")];
        let mut node2target = BTreeMap::new();
        for &ti in pass_rev.wires.values() {
            let TermInfo::PassFar(wf) = ti else {
                unreachable!()
            };
            for tile in [0, 1] {
                let tile = NodeTileId::from_idx(tile);
                let Some(name) = naming.wires.get(&(tile, wf)) else {
                    continue;
                };
                ...
                if self.builder.db.wires.key(wf) == "DBL.W.0.2" {
                    twf = (
                        NodeTileId::from_idx(tile.to_idx() ^ 1),
                        self.builder
                            .db
        ...
        for tile in [0, 1] {
            let pass = &self.builder.db.terms[self.builder.db.get_term(&format!("MAIN.{dir}"))];
            let naming = &self.builder.ndb.node_namings[self.builder.ndb.get_node_naming("INT")];
            let mut wires = EntityPartVec::new();
            for wt in pass.wires.ids() {
                let tile = NodeTileId::from_idx(tile);
                let Some(name) = naming.wires.get(&(tile, wt)) else {
                    continue;
                };
                let node = self.builder.rd.lookup_wire_force(int_xy, name);
    ...
    fn fill_io_term_short(&mut self, xy_w: Coord, xy_e: Coord) {
        ...
        let pass_w = &self.builder.db.terms[self.builder.db.get_term("MAIN.W")];
        ...
        let pass_e = &self.builder.db.terms[self.builder.db.get_term("MAIN.E")];
            ...
            [(Dir::W, xy_e, xy_w, 0, 1), (Dir::E, xy_w, xy_e, 1, 0)]
        {
            ...
            for &ti in pass.wires.values() {
                ...
                let name =
                    if let Some(name) = naming.wires.get(&(NodeTileId::from_idx(tile_from), wf)) {
                        name
                    } else if let Some(&owf) = switch_tile[tile_from].get(wf) {
                        if let Some(name) = naming
                            .wires
                            .get(&(NodeTileId::from_idx(tile_from ^ 1), owf))
                        {
                            name
                        } else {
                            continue;
            ...
            for wt in pass.wires.ids() {
                let name = if let Some(name) =
                    naming.wires.get(&(NodeTileId::from_idx(tile_to), wt))
                {
                    name
                } else if let Some(&owt) = switch_tile[tile_to].get(wt) {
                    if let Some(name) = naming.wires.get(&(NodeTileId::from_idx(tile_to ^ 1), owt))
                    {
                        name
                    } else {
                        continue;
    ...
    fn fill_io_term_long(&mut self, xy_w: Coord, xy_e: Coord) {
        for (dir, xy_to, xy_from) in [(Dir::W, xy_e, xy_w), (Dir::E, xy_w, xy_e)] {
            let pass = &self.builder.db.terms[self.builder.db.get_term(&format!("MAIN.L{dir}"))];
            let naming = &self.builder.ndb.node_namings[self.builder.ndb.get_node_naming("INT")];
            let mut node2target = BTreeMap::new();
            for &ti in pass.wires.values() {
                let TermInfo::PassFar(wf) = ti else {
                ...
                let tile = NodeTileId::from_idx(0);
                let Some(name) = naming.wires.get(&(tile, wf)) else {
                    continue;
                };
                let node = self.builder.rd.lookup_wire_force(xy_from, name);
    ...
    fn fill_tiles_bram(&mut self) {
        if let Some(&xy) = self.builder.rd.tiles_by_kind_name("BRAM").iter().next() {
            ...
            let mut xn = self.builder.xnode("BRAM", "BRAM", xy).num_tiles(5);
            for i in 0..5 {
                xn = xn
    ...
    fn fill_tiles_dsp(&mut self) {
        if let Some(&xy) = self.builder.rd.tiles_by_kind_name("DSP").iter().next() {
            ...
            let mut xn = self.builder.xnode("DSP", "DSP", xy).num_tiles(5);
            for i in 0..5 {
                xn = xn
    ...
    fn fill_tiles_bli(&mut self) {
            ...
            .next()
        {
            ...
            let bels = [
                self.builder
                    .bel_xy("BLI_HBM_APB_INTF", "BLI_HBM_APB_INTF", 0, 0),
                self.builder
                    .bel_xy("BLI_HBM_AXI_INTF", "BLI_HBM_AXI_INTF", 0, 0),
            ];
            let mut xn = self.builder.xnode("BLI", "BLI", xy).num_tiles(15);
            for i in 0..15 {
                xn = xn
                    .ref_int_side(xy.delta(-2, i as i32), Dir::E, i)
                    .ref_single(xy.delta(-1, i as i32), i, intf);
    ...
    fn fill_tiles_uram(&mut self) {
        for tkn in ["URAM_URAM_FT", "URAM_URAM_DELAY_FT"] {
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                let mut bels = vec![];
                for i in 0..4 {
                    let mut bel = self.builder.bel_xy(format!("URAM{i}"), "URAM288", 0, i);
                    let buf_cnt = match i {
                        0 => 1,
                        _ => 0,
                    };
                ...
                let mut xn = self.builder.xnode("URAM", "URAM", xy).num_tiles(30);
                for i in 0..15 {
                    xn = xn
                        .ref_int_side(xy.delta(-2, i as i32), Dir::E, i)
    ...
    fn fill_tiles_hard(&mut self) {
        for (kind, tkn, bk) in [
            ...
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                let mut xn = self.builder.xnode(kind, naming, xy).num_tiles(120);
                for i in 0..60 {
                    xn = xn
            ...
            .next()
        {
            ...
            let bel = self.builder.bel_xy("DFE_B", "DFE_B", 0, 0);
            let mut xn = self.builder.xnode("DFE_B", "DFE_B", xy).num_tiles(60);
            for i in 0..60 {
                xn = xn
        ...
        if let Some(&xy) = self.builder.rd.tiles_by_kind_name("FE_FE_FT").iter().next() {
            ...
            let intf = self.builder.ndb.get_node_naming("INTF.W.PCIE");
            let bel = self.builder.bel_xy("FE", "FE", 0, 0);
            let mut xn = self.builder.xnode("FE", "FE", xy).num_tiles(60);
            for i in 0..60 {
                xn = xn
    ...
    fn fill_tiles_ps(&mut self) {
        if let Some(&xy) = self.builder.rd.tiles_by_kind_name("PSS_ALTO").iter().next() {
            ...
            let intf = self.builder.ndb.get_node_naming("INTF.PSS");
            let mut bel = self.builder.bel_xy("PS", "PS8", 0, 0).pins_name_only(&[
                "DP_AUDIO_REF_CLK",
                "DP_VIDEO_REF_CLK",
                "DDR_DTO0",
                "DDR_DTO1",
            ...
            let mut xn = self.builder.xnode("PS", "PS", xy).num_tiles(180);
            for i in 0..180 {
                xn = xn
            ...
            .next()
        {
            ...
            let mut xn = self.builder.xnode("VCU", "VCU", xy).num_tiles(60);
            for i in 0..60 {
                xn = xn
                    .ref_int_side(int_r_xy.delta(0, (i + i / 30) as i32), Dir::W, i)
                    .ref_single(int_r_xy.delta(-1, (i + i / 30) as i32), i, intf);
        ...
        for tkn in [
            ...
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                let rclk_int = self.builder.ndb.get_node_naming("RCLK_INT");
                let mut bels = vec![];
                for i in 0..24 {
                    bels.push(
                        self.builder
    ...
    fn fill_tiles_cfg(&mut self) {
        for (kind, tkn, bkind) in [
            ...
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                let intf = self.builder.ndb.get_node_naming("INTF.E.PCIE");
                let bels = [
                    self.builder.bel_xy("CFG", bkind, 0, 0),
                    self.builder
                        .bel_xy("ABUS_SWITCH.CFG", "ABUS_SWITCH", 0, 0)
                        .pins_name_only(&["TEST_ANALOGBUS_SEL_B"]),
                ];
                let mut xn = self.builder.xnode(kind, kind, xy).num_tiles(60);
                for i in 0..60 {
                    xn = xn
                        .ref_int_side(int_xy.delta(0, (i + i / 30) as i32), Dir::E, i)
                        .ref_single(int_xy.delta(1, (i + i / 30) as i32), i, intf)
        ...
        for tkn in ["CFGIO_IOB20", "CFGIOLC_IOB20_FT"] {
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                let bels = [
                    ...
                    self.builder.bel_xy("CFGIO_SITE", "CFGIO_SITE", 0, 0),
                ];
        ...
        if let Some(&xy) = self.builder.rd.tiles_by_kind_name("AMS").iter().next() {
            let int_xy = self.builder.walk_to_int(xy, Dir::W, false).unwrap();
            let intf = self.builder.ndb.get_node_naming("INTF.E.PCIE");
            let mut bel = self.builder.bel_xy("SYSMON", "SYSMONE4", 0, 0);
            for i in 0..16 {
                bel = bel
                    .pin_name_only(&format!("VP_AUX{i}"), 1)
                    .pin_name_only(&format!("VN_AUX{i}"), 1);
            }
            let mut xn = self.builder.xnode("AMS", "AMS", xy).num_tiles(30);
            for i in 0..30 {
                xn = xn
                    .ref_int_side(int_xy.delta(0, (i + i / 30) as i32), Dir::E, i)
                    .ref_single(int_xy.delta(1, (i + i / 30) as i32), i, intf)
    ...
    fn fill_tiles_hdio(&mut self) {
        for tkn in ["HDIO_BOT_RIGHT", "HDIO_TOP_RIGHT"] {
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                bels.push(self.builder.bel_xy("HDLOGIC_CSSD", "HDLOGIC_CSSD", 0, 0));
                if is_bot {
                    bels.push(self.builder.bel_xy("HDIO_VREF", "HDIO_VREF", 0, 0));
                } else {
                    bels.push(self.builder.bel_xy("HDIO_BIAS", "HDIO_BIAS", 0, 0));
                }
                let kind = if is_bot { "HDIO_S" } else { "HDIO_N" };
                let mut xn = self.builder.xnode(kind, kind, xy).num_tiles(30);
                for i in 0..30 {
                    xn = xn
                        .ref_int_side(int_xy.delta(0, (i + i / 30) as i32), Dir::E, i)
                        .ref_single(int_xy.delta(1, (i + i / 30) as i32), i, intf)
        ...
        for (kind, tkn, side) in [
            ("HDIOLC_S", "HDIOLC_HDIOL_BOT_LEFT_FT", Dir::W),
            ("HDIOLC_N", "HDIOLC_HDIOL_TOP_LEFT_FT", Dir::W),
            ("HDIOLC_S", "HDIOLC_HDIOL_BOT_RIGHT_CFG_FT", Dir::E),
            ("HDIOLC_N", "HDIOLC_HDIOL_TOP_RIGHT_CFG_FT", Dir::E),
            ("HDIOLC_S", "HDIOLC_HDIOL_BOT_RIGHT_AUX_FT", Dir::E),
            ("HDIOLC_N", "HDIOLC_HDIOL_TOP_RIGHT_AUX_FT", Dir::E),
        ] {
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                let int_xy = self.builder.walk_to_int(xy, !side, false).unwrap();
                let intf = self.builder.ndb.get_node_naming(if side == Dir::W {
                    "INTF.W.IO"
                } else {
                    "INTF.E.PCIE"
                });
                ...
                for i in 0..2 {
                    bels.push(
                        self.builder
                            .bel_xy(format!("HDIO_VREF{i}"), "HDIO_VREF", 0, i),
                    );
                }
                bels.push(self.builder.bel_xy("HDIO_BIAS", "HDIO_BIAS", 0, 0));
                let mut xn = self.builder.xnode(kind, tkn, xy).num_tiles(30);
                for i in 0..30 {
                    xn = xn
        ...
        for (naming, tkn, side) in [
            ("RCLK_HDIOLC_L", "RCLK_RCLK_HDIOL_L_FT", Dir::W),
            ("RCLK_HDIOLC_R", "RCLK_RCLK_HDIOL_R_FT", Dir::E),
        ] {
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                let top_xy = xy.delta(0, -30);
                ...
                let mut xn = self.builder.xnode("RCLK_HDIOLC", naming, xy).num_tiles(60);
                for i in 0..60 {
                    xn = xn
                        .ref_int_side(int_xy.delta(0, (i + i / 30) as i32), side, i)
                        .ref_single(
    ...
    fn fill_tiles_cmt(&mut self) {
        for (kind, naming, tkn, side) in [
            ...
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                let mut xn = self.builder.xnode(kind, naming, xy).num_tiles(60).ref_xlat(
                    int_xy.delta(0, 30),
                    if side == Dir::W {
    ...
    fn fill_tiles_xiphy(&mut self) {
        for (tkn, side) in [("XIPHY_BYTE_L", Dir::W), ("XIPHY_BYTE_RIGHT", Dir::E)] {
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                bels.push(bel);
                let mut bel = self.builder.bel_virtual("XIPHY_BYTE");
                for i in 0..6 {
                    bel = bel.extra_wire(format!("XIPHY_CLK{i}"), &[format!("GCLK_FT0_{i}")]);
                }
                bels.push(bel);
                let mut xn = self.builder.xnode("XIPHY", "XIPHY", xy).num_tiles(15);
                for i in 0..15 {
                    xn = xn
                        .ref_int_side(int_xy.delta(0, (i + i / 30) as i32), side, i)
                        .ref_single(
        ...
        for (tkn, naming) in [
            ...
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                let mut bel = self.builder.bel_virtual("RCLK_XIPHY");
                for i in 0..24 {
    ...
    fn fill_tiles_hpio(&mut self) {
        for (tkn, side) in [("HPIO_L", Dir::W), ("HPIO_RIGHT", Dir::E)] {
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                for i in 0..2 {
                    bels.push(
                        self.builder
                            .bel_xy(format!("HPIO_DCI{i}"), "HPIOB_DCI_SNGL", 0, i),
                    );
                }
                bels.push(
                    self.builder
                ...
                bels.push(self.builder.bel_xy("HPIO_BIAS", "BIAS", 0, 0));
                ...
                let mut xn = self.builder.xnode("HPIO", naming, xy).num_tiles(30);
                for i in 0..30 {
                    xn = xn
                        .ref_int_side(int_xy.delta(0, (i + i / 30) as i32), side, i)
                        .ref_single(
        ...
        for (tkn, side) in [("RCLK_HPIO_L", Dir::W), ("RCLK_HPIO_R", Dir::E)] {
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                for i in 0..7 {
                    bels.push(self.builder.bel_xy(
                        format!("ABUS_SWITCH.HPIO{i}"),
                        "ABUS_SWITCH",
                        if side == Dir::W {
                            i
                        } else {
                            [0, 6, 1, 3, 2, 4, 5][i as usize]
                        },
                        0,
                    ));
                }
                ...
                bels.push(
                    self.builder
                        .bel_xy("HPIO_RCLK_PRBS", "HPIO_RCLK_PRBS", 0, 0),
                );
                let mut xn = self
                    .builder
                    .xnode("RCLK_HPIO", "RCLK_HPIO", xy)
                    .num_tiles(60);
                for i in 0..60 {
                    xn = xn
                        .ref_int_side(int_xy.delta(0, (i + i / 30) as i32), side, i)
    ...
    fn fill_tiles_rclk(&mut self) {
        ...
        for (node, tkn) in [
            ...
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                let bels = vec![
                    self.builder
                        .bel_xy("BUFCE_ROW0", "BUFCE_ROW_FSR", 0, 0)
                        .pins_name_only(&["CLK_IN", "CLK_OUT", "CLK_OUT_OPT_DLY"])
        ...
        for tkn in [
            ...
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                for i in 0..2 {
                    bels.push(
                        self.builder
                            .bel_xy(format!("BUFCE_ROW{i}"), "BUFCE_ROW_FSR", i, 0)
                            .pins_name_only(&["CLK_IN", "CLK_OUT", "CLK_OUT_OPT_DLY"])
        ...
        for (node, tkn) in [
            ...
            if let Some(&xy) = self.builder.rd.tiles_by_kind_name(tkn).iter().next() {
                ...
                for (i, x, y) in [(0, 0, 0), (1, 0, 1), (2, 1, 0)] {
                    bels.push(
                        self.builder
                            .bel_xy(format!("VBUS_SWITCH{i}"), "VBUS_SWITCH", x, y),
                    );
                }
                bels.push(
                    self.builder
