        ...
        if slot_name.starts_with("IO") && grid.kind == ChipKind::Xc4000H {
            (
                ...
                TileWireCoord::new_idx(
                    0,
                    backend.edev.db.get_wire(if slot == bels::IO0 {
                        "OUT.HIOB0.I"
                    } else {
                        "OUT.HIOB2.I"
                    }),
...
fn apply_imux_finish<'a>(
    ...
    if bels::IO.contains(&slot) && (pin == "O2" || pin == "O1") {
        ...
        let opin = backend.edev.resolve_tile_wire(tcrd, opin).unwrap();
        fuzzer = fuzzer
        ...
        if tile.starts_with("CLB") {
            ctx.build()
                ...
                .prop(ClbSpecialMux::new(
                    "F4",
                    "CIN",
                    TileWireCoord::new_idx(0, backend.edev.db.get_wire("IMUX.CLB.F4")),
                    0,
                    0,
                ))
                .commit();
            ...
            )
        {
            ctx.build()
                ...
                .prop(ClbSpecialMux::new(
                    "G3",
                    "CIN",
                    TileWireCoord::new_idx(0, backend.edev.db.get_wire("IMUX.CLB.G3")),
                    -1,
                    0,
                ))
                .commit();
            ...
            )
        {
            ctx.build()
                ...
                .prop(ClbSpecialMux::new(
                    "G2",
                    "COUT0",
                    TileWireCoord::new_idx(0, backend.edev.db.get_wire("IMUX.CLB.G2")),
                    0,
                    1,
                ))
                .commit();
...
pub fn collect_fuzzers(ctx: &mut CollectorCtx) {
    ...
    for (_, tcname, tcls) in &intdb.tile_classes {
        for (bslot, bel) in &tcls.bels {
                ...
                match item {
                    SwitchBoxItem::Mux(mux) => {
                        ...
                        if let Some((prefix, bel, pin, rect_idx)) = iob_o {
                            if ctx.edev.chip.kind == ChipKind::Xc4000H {
                                ...
                                ctx.insert(tcname, "INT", mux_name, item);
                            } else {
                            ...
                            let imux_i = TileWireCoord::new_idx(
                                0,
                            ...
                            ctx.insert(tcname, "INT", mux_name_i, item_i);
                            ...
                            ctx.insert(tcname, "INT", mux_name, item_t);
                            ctx.insert(tcname, format!("TBUF{idx}"), "DRIVE1", xlat_bit(!common));
                        } else {
                            let mut inps = vec![];
                            let mut got_empty = false;
                            for &wire_from in mux.src.keys() {
                    ...
                    SwitchBoxItem::BiPass(pass) => {
                        ...
                        for (wdst, wsrc) in [(pass.a, pass.b), (pass.b, pass.a)] {
                            ...
                            ctx.insert(tcname, bel, &name, xlat_bit(diff));
                        }
                    }
                    _ => unreachable!(),
                }
            ...
            for ((prefix, bel), diffs) in hiob_o_diffs {
                ...
                if prefix == tcname {
                    ctx.insert(tcname, bel, "MUX.O", item);
                } else {
                    for tile in intdb.tile_classes.keys() {
                        if tile.starts_with(prefix) {
                            ctx.insert(tile, bel, "MUX.O", item.clone());
                        }
                    }
                }
            }
    ...
    for ((prefix, bel), mut diffs) in iob_o_diffs {
        ...
        for tile in intdb.tile_classes.keys() {
            if tile.starts_with(prefix) {
                ctx.insert(tile, bel, "INV.T", item.clone());
            }
        }
        ...
        for tile in intdb.tile_classes.keys() {
            if tile.starts_with(prefix) {
                ctx.insert(tile, bel, "INV.OFF_D", item.clone());
            }
        }
        ...
        for tile in intdb.tile_classes.keys() {
            if tile.starts_with(prefix) {
                ctx.insert(tile, bel, "MUX.OFF_D", item.clone());
            }
        }
        ...
        for tile in intdb.tile_classes.keys() {
            if tile.starts_with(prefix) {
                ctx.insert(tile, bel, "OMUX", item.clone());
            }
        }
        ...
        for tile in intdb.tile_classes.keys() {
            if tile.starts_with(prefix) {
                ctx.insert(tile, bel, "OFF_USED", item.clone());
            }
        }
    }
}
