...
pub fn make_int_db(rd: &Part) -> (IntDb, NamingDb) {
    ...
    for (tkn, naming) in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            if naming == "HCLK.QBUF" {
                ...
                builder
                    .xtile(tslots::HROW, "HCLK_QBUF", "HCLK_QBUF", xy)
                    .num_tiles(0)
                    .bel(bel)
                    .extract();
            }
    ...
    for (tkn, naming) in [("HCLK_CMT_BOT", "CMT.BOT"), ("HCLK_CMT_TOP", "CMT.TOP")] {
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            bels.push(bel);
            let mut xn = builder
                .xtile(tslots::BEL, "CMT", naming, xy_bot)
                .num_tiles(40)
                .raw_tile(xy_top)
                .raw_tile(xy);
            for i in 0..20 {
    ...
    for tkn in ["CMT_PMVA", "CMT_PMVA_BELOW"] {
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xtile(tslots::BEL, "PMVIOB", tkn, xy)
                .num_tiles(2)
    ...
    for tkn in ["CMT_PMVB_BUF_BELOW", "CMT_PMVB_BUF_ABOVE"] {
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xtile(tslots::BEL, "GCLK_BUF", "GCLK_BUF", xy)
                .num_tiles(0)
                .bel(bel)
                .extract();
        }
    ...
    for (tkn, naming) in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xtile(tslots::CLK, "MGT_BUF", naming, xy)
                .num_tiles(0)
                .bel(bel)
                .extract();
        }
