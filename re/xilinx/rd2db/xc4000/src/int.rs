...
fn extract_bot(
    ...
    for (nn, tkn) in [
        ...
        for &crd in builder.rd.tiles_by_kind_name(tkn) {
            ...
            let mut bels = vec![];
            for i in 0..2 {
                bels.push(builder.bel_indexed(bels::IO[i], "IOB", i as u8 + 1))
            }
            if builder.rd.family != "spartanxl" {
                for i in 0..3 {
                    bels.push(builder.bel_indexed(bels::DEC[i], "DEC", i as u8 + 1))
                }
            }
...
fn extract_rt(builder: &mut IntBuilder, imux_wires: &[WireId], imux_nw: &[NodeWireId]) {
    ...
    for (nn, tkn) in [
        ...
        for &crd in builder.rd.tiles_by_kind_name(tkn) {
            ...
            let mut bels = vec![];
            for i in 0..2 {
                let mut bel = builder.bel_indexed(bels::IO[i], "IOB", i as u8 + 1);
...
fn extract_ur(builder: &mut IntBuilder, imux_wires: &[WireId], imux_nw: &[NodeWireId]) {
    ...
    for &crd in builder.rd.tiles_by_kind_name("UR") {
        ...
        match &*builder.rd.family {
            ...
            "xc4000e" => {
                for i in 0..4 {
                    bels.push(builder.bel_indexed(bels::PULLUP_DEC_H[i], "PULLUP", i as u8 + 1));
                }
                for i in 0..4 {
                    bels.push(builder.bel_indexed(bels::PULLUP_DEC_V[i], "PULLUP", i as u8 + 5));
                }
                bels.extend([
                    builder
