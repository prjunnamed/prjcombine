...
use crate::xilinx::geomdb::{Grid, GridCell, GridRanges, Dir, Orient, TCWire, WireConn, PipInversion, TileTran, TileMux, TileMuxBranch};
use crate::xilinx::geomdb::builder::GridBuilder;
use crate::xilinx::geomraw::{PartRaw, ExtractClass, Extract, ExtractTie, RawPip, RawPipDirection};
use super::cfg::{RawSiteSlot, GridSnap, TileAnchor, IntTermInfo};
use super::builder::{GeomBuilder, ExtractInfo};
...
impl<'a> PartBuilder<'a> {
    pub fn new(name: String, rd: &'a rawdump::Part, geob: &GeomBuilder) -> Self {
        ...
        PartBuilder {
            grid: Grid {
                name,
                grid: Array2::from_elem([width, height], GridCell {
                    tiles: geob.geomdb.tile_slots.iter().map(|_| None).collect(),
                    ports: geob.geomdb.port_slots.iter().map(|_| None).collect(),
                }),
                columns: Vec::new(),
                vert_bus: (0..geob.geomdb.vert_bus.len()).map(|_| Default::default()).collect(),
                horiz_bus: (0..geob.geomdb.horiz_bus.len()).map(|_| Default::default()).collect(),
                tiles: Vec::new(),
            },
            raw: PartRaw {
                tiles: HashMap::new(),
            ...
            int_r2g_x, int_r2g_y,
            rd,
            slot_kind_lut: rd.slot_kinds.iter().enumerate().map(|(i, s)| (s.clone(), i as u16)).collect(),
    ...
    pub fn map_site_slot(&self, slot: &RawSiteSlot) -> rawdump::TkSiteSlot {
        match slot {
            ...
            &RawSiteSlot::Indexed(ref s, i) => rawdump::TkSiteSlot::Indexed(self.slot_kind_lut[s], i),
    ...
    pub fn make_site_slot(&self, slot: rawdump::TkSiteSlot) -> RawSiteSlot {
        match slot {
            rawdump::TkSiteSlot::Single(s) => RawSiteSlot::Single(self.rd.slot_kinds[s as usize].clone()),
            rawdump::TkSiteSlot::Indexed(s, i) => RawSiteSlot::Indexed(self.rd.slot_kinds[s as usize].clone(), i),
            rawdump::TkSiteSlot::Xy(s, x, y) => RawSiteSlot::Xy(self.rd.slot_kinds[s as usize].clone(), x, y),
        }
    }
    ...
    pub fn extract(&mut self, geob: &mut GeomBuilder, info: ExtractInfo, tiles: Vec<((usize, usize), Vec<rawdump::Coord>)>) {
        ...
        for (xy, crds) in tiles {
            ...
            self.raw.tiles.insert((xy, tcls), Extract {
                cls: ecls,
                raw_tiles: raw_tiles.iter().copied().map(|tile| tile.name.clone()).collect(),
                raw_sites: site_slots.iter().map(|&(ref rslot, rtidx)| {
            ...
            for (i, crd) in crds.into_iter().enumerate() {
                self.extracted_tiles.entry(crd).or_default().push(((xy, tcls), i));
            }
        }
    }
    ...
    pub fn fill_int_tiles(&mut self, geob: &mut GeomBuilder) {
        ...
        for tt in geob.cfg.int_tiles.iter() {
            ...
            if let Some(tk) = self.rd.tile_kinds.get(tt.raw_tile) {
                let tiles: Vec<_> = tk.tiles.iter().copied().map(|crd| {
        ...
        for (mut info, tiles) in extracts {
            ...
            for (&a, ins) in tmuxes[&info.cls.tcls].iter().sorted_by_key(|(&a, _)| a) {
                tile_muxes.push(TileMux {
                    wire_out: a,
                    branches: ins.iter().copied().sorted().map(|b| TileMuxBranch {
                        wire_in: b,
            ...
            if let Some(tk) = self.rd.tile_kinds.get(rt) {
                'outer: for crd in tk.tiles.iter().copied() {
                    for (dx, dy, ert) in anchor.extra_raw.iter().copied() {
                        ...
                        if erx < 0 || ery < 0 || erx >= (self.rd.width as isize) || ery >= (self.rd.height as isize) {
                            continue 'outer;
                        }
                        let tile = &self.rd.tiles[&rawdump::Coord {x: erx as u16, y: ery as u16}];
    ...
    pub fn find_anchor_gx_set(&self, anchor: &TileAnchor) -> HashSet<usize> {
        self.find_anchors(anchor).into_iter().map(|(_, (x, _))| x).collect()
    }

    pub fn find_anchor_gy_set(&self, anchor: &TileAnchor) -> HashSet<usize> {
        self.find_anchors(anchor).into_iter().map(|(_, (_, y))| y).collect()
    }

    pub fn walk_grid(&self, geob: &GeomBuilder, x: usize, y: usize, dx: isize, dy: isize) -> (usize, usize) {
    ...
    pub fn get_int_term_xy(&self, geob: &GeomBuilder, term: &IntTermInfo, crd: rawdump::Coord) -> Vec<((usize, usize), String)> {
    ...
    pub fn fill_int_terms(&mut self, geob: &GeomBuilder) {
        for term in geob.cfg.int_terms.iter() {
            if let Some(tk) = self.rd.tile_kinds.get(term.raw_tile) {
                let mut exts : HashMap<String, Vec<(rawdump::Coord, (usize, usize))>> = HashMap::new();
                ...
                for (name, tiles) in exts {
                    ...
                    for (_crd, xy) in tiles {
                        // XXX extraction here?
                        self.grid.fill_port_term(&geob.geomdb, xy, geob.geomdb.ports.idx(term.name));
                        if term.needs_tile {
                            self.grid.fill_tile(&geob.geomdb, xy, geob.geomdb.tiles.idx(term.name));
                        }
                    }
                }
            }
            ...
            if let Some(tk) = self.rd.tile_kinds.get(buf.raw_tile) {
                for crd in tk.tiles.iter().copied() {
                    ...
                    for (xya, xyb, _raw) in tiles {
                        // XXX extraction here?
                        self.grid.fill_port_pair(&geob.geomdb, xya, xyb, int.pcls_buf[buf.name]);
                        if buf.needs_tile {
                            self.grid.fill_tile(&geob.geomdb, xyb, geob.geomdb.tiles.idx(buf.name));
                        }
                    }
                }
            }
        ...
        for dbuf in geob.cfg.int_dbufs.iter() {
            if let Some(tk) = self.rd.tile_kinds.get(dbuf.raw_tile_a) {
                'outer: for crd_a in tk.tiles.iter().copied() {
                        ...
                        Orient::H => {
                            match dbuf.span {
                                None => {
                                    let c_a = self.walk_grid(geob, gx_l, gy_u, -1, 0);
                                    let c_b = self.walk_grid(geob, gx_r, gy_u, 1, 0);
                                    tiles.push((c_a, c_b, format!("{}.{}", dbuf.raw_tile_a, dbuf.raw_tile_b)));
                                },
                                Some((n, p)) => {
                                    assert!(gy_u >= p);
                                    for d in 0..n {
                                        ...
                                        let c_a = self.walk_grid(geob, gx_l, y, -1, 0);
                                        let c_b = self.walk_grid(geob, gx_r, y, 1, 0);
                                        tiles.push((c_a, c_b, format!("{}.{}[{}]", dbuf.raw_tile_a, dbuf.raw_tile_b, d)));
                                    }
                                },
                            }
                        },
                    ...
                    for (xya, xyb, _raw) in tiles {
                        // XXX extraction here?
                        self.grid.fill_port_pair(&geob.geomdb, xya, xyb, int.pcls_dbuf[dbuf.name]);
                        if dbuf.needs_tile {
                            self.grid.fill_tile(&geob.geomdb, xya, int.tcls_dbuf[dbuf.name].0);
                            self.grid.fill_tile(&geob.geomdb, xyb, int.tcls_dbuf[dbuf.name].1);
                        }
                    }
                }
            }
    ...
    pub fn fill_int_passes(&mut self, geob: &GeomBuilder) {
        let int = geob.get_int();
        let mut specials: HashMap<((usize, usize), (usize, usize)), BTreeSet<&'static str>> = HashMap::new();
        ...
        for (xya, xyb) in all_pairs {
            ...
            match specials.get(&(xya, xyb)) {
                ...
                Some(specs) => {
                    ...
                    self.grid.fill_port_pair(&geob.geomdb, xya, xyb, int.pcls_pass[spec]);
                },
            }
        }
    }
    ...
    pub fn fill_bus(&mut self, geob: &GeomBuilder, orient: Orient, bus: &str, endpoints: Vec<usize>, midpoints: Vec<usize>) {
    ...
    pub fn fill_srcbrk_bus(&mut self, geob: &GeomBuilder, orient: Orient, bus: &str, src: impl Iterator<Item=usize>, brk: impl Iterator<Item=usize>) {
        let end = match orient {
            Orient::H => self.width(),
            Orient::V => self.height(),
        };
        let ccs: Vec<_> = src.sorted().dedup().collect();
        let cbs: Vec<_> = iter::once(0).chain(brk.sorted()).chain(iter::once(end)).dedup().collect();
    ...
    pub fn fill_srcbrk_bus_split(&mut self, geob: &GeomBuilder, orient: Orient, bus: &str, src: impl Iterator<Item=usize>, brk: impl Iterator<Item=usize>) {
        let end = match orient {
            Orient::H => self.width(),
            Orient::V => self.height(),
        };
