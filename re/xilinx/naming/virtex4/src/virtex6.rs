    ...
    for (tcrd, tile) in edev.tiles() {
        ...
        match tile.class {
            ...
            tcls::DSP => {
                ...
                ntile.add_bel(defs::bslots::TIEOFF_DSP, format!("TIEOFF_X{tx}Y{y}"));
            }
            tcls::IO => {
            ...
            tcls::HCLK_IO => {
                ...
                ntile.add_bel(
                    defs::bslots::BUFIO[2],
                    format!("BUFIODQS_X{iox}Y{y}", y = hy * 4),
                );
                ntile.add_bel(
                    defs::bslots::BUFIO[3],
                ...
                ntile.add_bel(
                    defs::bslots::BUFR[0],
                    format!("BUFR_X{iox}Y{y}", y = hy * 2 + 1),
                );
                ntile.add_bel(
                    defs::bslots::BUFR[1],
                    format!("BUFR_X{iox}Y{y}", y = hy * 2),
                );
                ntile.add_bel(
                    defs::bslots::BUFO[0],
                ...
                ntile.add_bel(defs::bslots::DCI, format!("DCI_X{iox}Y{hy}"));
            }
            tcls::CMT => {
                ...
                for (i, slots) in [defs::bslots::BUFHCE_W, defs::bslots::BUFHCE_E]
                    .into_iter()
                    .enumerate()
                {
                    for j in 0..12 {
                ...
                ntile.add_bel(
                    defs::bslots::MMCM[0],
                    format!("MMCM_ADV_X{bx}Y{y}", y = by * 2),
                );
                ntile.add_bel(
                    defs::bslots::MMCM[1],
                    format!("MMCM_ADV_X{bx}Y{y}", y = by * 2 + 1),
                );
                ntile.add_bel(defs::bslots::PPR_FRAME, format!("PPR_FRAME_X{bx}Y{by}"));
            }
            tcls::PMVIOB => {
                let naming = if row < chip.row_bufg() {
                    "CMT_PMVA_BELOW"
                ...
                ntile.add_bel(defs::bslots::PMVIOB_CLK, format!("PMVIOB_X{bx}Y{by}"));
            }
            tcls::CMT_BUFG_S => {
                let ntile = namer.ngrid.name_tile(
                    tcrd,
            ...
            tcls::CFG => {
                ...
                ntile.add_bel(defs::bslots::ICAP[0], "ICAP_X0Y0".to_string());
                ntile.add_bel(defs::bslots::ICAP[1], "ICAP_X0Y1".to_string());
                ntile.add_bel(defs::bslots::PMV_CFG[0], "PMV_X0Y0".to_string());
                ntile.add_bel(defs::bslots::PMV_CFG[1], "PMV_X0Y1".to_string());
                ntile.add_bel(defs::bslots::STARTUP, "STARTUP_X0Y0".to_string());
                ntile.add_bel(defs::bslots::CAPTURE, "CAPTURE_X0Y0".to_string());
                ntile.add_bel(defs::bslots::FRAME_ECC, "FRAME_ECC".to_string());
                ntile.add_bel(defs::bslots::EFUSE_USR, "EFUSE_USR_X0Y0".to_string());
                ntile.add_bel(defs::bslots::USR_ACCESS, "USR_ACCESS_X0Y0".to_string());
                ntile.add_bel(defs::bslots::DNA_PORT, "DNA_PORT_X0Y0".to_string());
                ntile.add_bel(defs::bslots::DCIRESET, "DCIRESET_X0Y0".to_string());
                ntile.add_bel(
                    defs::bslots::CFG_IO_ACCESS,
                    "CFG_IO_ACCESS_X0Y0".to_string(),
                );
            ...
            tcls::GTH => {
                ...
                ntile.add_bel(defs::bslots::GTH_QUAD, format!("GTHE1_QUAD_X{gx}Y{gthy}"));
                ntile.add_bel(
                    defs::bslots::BUFDS[0],
