...
pub fn make_int_db(rd: &Part) -> (IntDb, NamingDb) {
    ...
    for (tkn, naming, idx) in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xnode(tslots::IOB, "IOB", naming, xy)
                .num_tiles(0)
                .bels(bels)
                .extract();
        }
    ...
    for tkn in ["REGH_LIOI_INT", "REGH_LIOI_INT_BOT25"] {
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xnode(tslots::BEL, "PCILOGICSE", "PCILOGICSE_L", xy)
    ...
    for tkn in ["REGH_RIOI", "REGH_RIOI_BOT25"] {
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            let bel = builder
                ...
                .pin_name_only("TRDY", 1);
            builder
                .xnode(tslots::BEL, "PCILOGICSE", "PCILOGICSE_R", xy)
    ...
    for (tkn, naming) in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xnode(tslots::IOI_CLK, "BTIOI_CLK", naming, xy)
                .num_tiles(0)
                .bel(bel)
                .extract();
        }
    ...
    for (tkn, trunk_naming, is_trunk_b, v_naming, is_v_dn) in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xnode(tslots::PCI_CE_BUF, "PCI_CE_V_BUF", v_naming, xy)
                .num_tiles(0)
                .bel(bel)
                .extract();
        }
    ...
    for tkn in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xnode(tslots::PCI_CE_BUF, "PCI_CE_SPLIT", "PCI_CE_SPLIT", xy)
                .num_tiles(0)
                .bel(bel)
                .extract();
        }
    ...
    for (tkn, naming, lr) in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xnode(tslots::IOI_CLK, "LRIOI_CLK", naming, xy)
                .raw_tile(xy) // dummy
                .num_tiles(0)
                .bel(bel)
                .bel(bel_term)
    ...
    for (tkn, naming) in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xnode(tslots::PCI_CE_BUF, "PCI_CE_H_BUF", naming, xy)
                .num_tiles(0)
                .bel(bel)
                .extract();
        }
    ...
    for tkn in ["DSP_HCLK_GCLK_FOLD", "GTPDUAL_DSP_FEEDTHRU"] {
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xnode(tslots::CLK_BUF, "HCLK_H_MIDBUF", tkn, xy)
                .num_tiles(0)
                .bel(bel)
                .extract();
        }
    ...
    if let Some(&xy) = rd.tiles_by_kind_name("REG_V_HCLK").iter().next() {
        ...
        bels.push(bel);
        builder
            .xnode(tslots::HCLK_ROW, "HCLK_ROW", "HCLK_ROW", xy)
            .num_tiles(0)
            .bels(bels)
            .extract();
    }
    ...
    for tkn in ["REG_V_HCLKBUF_BOT", "REG_V_HCLKBUF_TOP"] {
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xnode(tslots::CLK_BUF, "HCLK_V_MIDBUF", "HCLK_V_MIDBUF", xy)
                .num_tiles(0)
                .bel(bel)
                .extract();
        }
    ...
    for tkn in ["REG_V_MIDBUF_BOT", "REG_V_MIDBUF_TOP"] {
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xnode(tslots::CLK_BUF, "CKPIN_V_MIDBUF", tkn, xy)
                .num_tiles(0)
                .bel(bel)
                .extract();
        }
    ...
    for tkn in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xnode(tslots::CLK_BUF, "CKPIN_H_MIDBUF", "CKPIN_H_MIDBUF", xy)
                .num_tiles(0)
                .bel(bel)
                .extract();
        }
    ...
    for (tkn, e, bio2, bpll) in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            let mut xn = builder.xnode(tslots::REG, tkn, tkn, xy);
            match tkn {
                "REG_L" => {
                    xn = xn
                        .num_tiles(2)
    ...
    for (tkn, naming, lr, is_top) in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xnode(tslots::CLK_BUF, "CLKPIN_BUF", naming, xy)
                .num_tiles(0)
                .bel(bel)
                .extract();
        }
    ...
    for (tkn, bt, kind, slot, d0, d1, d2) in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            bels.push(bel);
            builder
                .xnode(tslots::BEL, "CMT_DCM", tkn, xy)
                .num_tiles(2)
            ...
            builder
                .xnode(tslots::CMT_BUF, kind, kind, xy)
                .num_tiles(0)
                .bel(bel)
                .extract();
        }
    ...
    for (tkn, bt, out) in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            builder
                .xnode(tslots::BEL, "CMT_PLL", tkn, xy)
                .num_tiles(2)
                .ref_single(xy.delta(-1, -2), 0, intf)
                .ref_single(xy.delta(-1, 0), 1, intf)
                .bel(bel_pll)
            ...
            if let Some(out) = out {
                ...
                builder
                    .xnode(tslots::CMT_BUF, node, node, xy)
                    .num_tiles(0)
                    .bel(bel)
                    .extract();
            } else {
                builder
                    .xnode(tslots::CMT_BUF, "PLL_BUFPLL_B", "PLL_BUFPLL_B", xy)
                    .num_tiles(0)
                    .extract();
                builder
                    .xnode(tslots::CMT_BUF, "PLL_BUFPLL_T", "PLL_BUFPLL_T", xy)
                    .num_tiles(0)
                    .extract();
            }
        }
