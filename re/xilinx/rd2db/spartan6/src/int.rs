...
pub fn make_int_db(rd: &Part) -> (IntDb, NamingDb) {
    ...
    for (nn, tkn, naming, is_bt) in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            for i in 0..2 {
                ...
                let mut bel = builder
                    .bel_xy(bels::ILOGIC[i], "ILOGIC", 0, i as u8)
                    .pins_name_only(&[
            ...
            for i in 0..2 {
                ...
                let bel = builder
                    .bel_xy(bels::OLOGIC[i], "OLOGIC", 0, i as u8)
                    .pins_name_only(&[
                        "CLK0",
                        "CLK1",
                        "IOCE",
            ...
            for i in 0..2 {
                ...
                let mut bel = builder
                    .bel_xy(bels::IODELAY[i], "IODELAY", 0, i as u8)
                    .pins_name_only(&[
                        "IOCLK0",
                        "IOCLK1",
                        "IDATAIN",
    ...
    if let Some(&xy) = rd.tiles_by_kind_name("REG_V_HCLK").iter().next() {
        ...
        for i in 0..2 {
            ...
            for j in 0..16 {
                bels.push(
                    builder
                        .bel_xy(slots[j], "BUFH", i * 3, (1 - i) * 16 + j as u8)
                        .pin_name_only("I", 1)
                        .pin_name_only("O", 1),
                );
            }
    ...
    if let Some(&xy) = rd.tiles_by_kind_name("CLKC").iter().next() {
        ...
        for i in 0..16 {
            bels.push(
                builder
                    .bel_xy(bels::BUFGMUX[i], "BUFGMUX", u8::from((i & 4) != 0), i as u8)
                    .pin_name_only("O", 1)
    ...
    for (tkn, bt, kind, slot, d0, d1, d2) in [
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            for i in 0..2 {
                let ii = 2 - i;
                let mut bel = builder
                    .bel_xy(bels::DCM[i], "DCM", 0, i as u8)
    ...
    for tkn in ["GTPDUAL_BOT", "GTPDUAL_TOP"] {
        ...
        if let Some(&xy) = rd.tiles_by_kind_name(tkn).iter().next() {
            ...
            for i in 0..2 {
                bels.push(
                    builder
                        .bel_xy(bels::BUFDS[i], "BUFDS", 0, i as u8)
                        .pins_name_only(&["I", "IB", "O"]),
                );
            }
            let mut bel = builder
