...
pub fn finish(geom: GeomDb, tiledb: TileDb) -> Database {
    ...
    for dev in &geom.devices {
        let prjcombine_xilinx_geom::Grid::Virtex(ref grid) =
        ...
        let disabled: BTreeSet<_> = dev
            ...
            .iter()
            .map(|&dis| {
                let prjcombine_xilinx_geom::DisabledPart::Virtex(dis) = dis else {
                    unreachable!()
                };
                dis
            })
        ...
        for devbond in dev.bonds.values() {
            let prjcombine_xilinx_geom::Bond::Virtex(ref bond) = geom.bonds[devbond.bond] else {
                unreachable!()
            };
