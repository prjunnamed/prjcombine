...
use crate::{
    ...
    fuzz::FuzzCtx,
    fuzz_enum, fuzz_multi_attr_dec, fuzz_multi_attr_hex, fuzz_one, fuzz_one_extras,
};
...
pub fn add_fuzzers<'a>(session: &mut Session<IseBackend<'a>>, backend: &IseBackend<'a>) {
    if let Some(ctx) = FuzzCtx::try_new(session, backend, "PCIE", "PCIE", TileBits::MainAuto) {
        ...
        fuzz_one_extras!(ctx, "DRP_MASK", "1", [(mode "PCIE_2_1")], [(pin "DRPWE")], extras);

        for &attr in PCIE_BOOL_ATTRS {
            fuzz_enum!(ctx, attr, ["FALSE", "TRUE"], [(mode "PCIE_2_1")]);
        }
        for &(attr, width) in PCIE_HEX_ATTRS {
            fuzz_multi_attr_hex!(ctx, attr, width, [(mode "PCIE_2_1")]);
        }
        for &(attr, width) in PCIE_DEC_ATTRS {
            fuzz_multi_attr_dec!(ctx, attr, width, [(mode "PCIE_2_1")]);
        }
    }
    if let Some(ctx) = FuzzCtx::try_new(session, backend, "PCIE3", "PCIE3", TileBits::Pcie3) {
        // always turns on the "bottom" bit even in the lower region — bug or intentional?
        fuzz_one!(ctx, "PRESENT", "1", [], [(mode "PCIE_3_0")]);
        let extras = vec![ExtraFeature::new(
        ...
        fuzz_one_extras!(ctx, "DRP_MASK", "1", [(mode "PCIE_3_0")], [(pin "DRPWE")], extras);

        for &attr in PCIE3_BOOL_ATTRS {
            fuzz_enum!(ctx, attr, ["FALSE", "TRUE"], [(mode "PCIE_3_0")]);
        }
        for &(attr, width) in PCIE3_HEX_ATTRS {
            fuzz_multi_attr_hex!(ctx, attr, width, [(mode "PCIE_3_0")]);
        }
        for &(attr, width) in PCIE3_DEC_ATTRS {
            fuzz_multi_attr_dec!(ctx, attr, width, [(mode "PCIE_3_0")]);
        }
    }
}
