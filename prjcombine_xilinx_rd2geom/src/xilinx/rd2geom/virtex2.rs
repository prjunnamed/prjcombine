...
use super::cfg::{
    GeomBuilderConfig, IntDoubleBufInfo, IntTermInfo, IntTileInfo, TieSiteInfo, TileAnchor,
    TileInfo,
};
use super::part::PartBuilder;
use super::RdGeomMakerImpl;
use crate::xilinx::geomdb::builder::{GeomDbBuilder, GridBuilder};
use crate::xilinx::geomdb::{Dir, GeomDb, Orient, TieState};
...
impl Virtex2GeomMaker {
    pub fn new(family: &str) -> Self {
        ...
        let mut tiles = vec![
            ...
            TileInfo::site_int_extra(
                "SITE_DCM_B",
                &["BRAM_IOIS", "ML_BRAM_IOIS"],
                vec![(0, -1, &["BBTERM"])],
            ),
            TileInfo::site_int_extra(
                "SITE_DCM_T",
    ...
    fn setup_int_ll(&mut self) {
        ...
        let lh: Vec<_> = (0..24)
            .map(|i| {
                ...
                self.builder
                    .register_int_wire(w, &[&format!("LH{}", i), &format!("LPPC_INT_LH{}", i)]);
                w
            })
            .collect();
        for i in 0..24 {
    ...
    fn setup_int_imux(&mut self) {
        ...
        let (pcls_bram_addr_n, pcls_bram_addr_s) = self.builder.geomdb.make_port_pair(
            ("BRAM_ADDR_N_PASS", "BRAM_ADDR_S_PASS"),
            (pslot_bram_addr_n, pslot_bram_addr_s),
        );
    ...
    fn setup_int_out(&mut self) {
        ...
        for i in 0..8 {
            let w = self.builder.make_int_out_wire(
            ...
            if self.is_2vp() {
                if i == 0 {
                    self.builder.register_tile_wire(
                        w,
                        &["INT_IOI.MK_T_IOIS"],
                        "IOIS_BREFCLK_SE",
                        (0, 0),
        ...
        for i in 2..24 {
            self.builder.make_int_out_wire(
                ...
                &[
                    ...
                    &if (8..16).contains(&i) {
                        format!("LRPPC_INT_PPC2{}", 15 - i)
                    } else {
                        format!("")
                    },
                    &if (8..16).contains(&i) {
                        format!("BPPC_INT_PPC2{}", 15 - i)
                    } else {
                        format!("")
                    },
                    &if (8..16).contains(&i) {
                        format!("TPPC_INT_PPC2{}", 15 - i)
                    } else {
                        format!("")
                    },
                    &if (8..16).contains(&i) {
                        format!("GIGABIT_INT_PPC2{}", 15 - i)
                    } else {
                        format!("")
                    },
                ],
            ...
            for j in 0..2 {
                self.builder.make_int_out_wire(
                    ...
                    &[
                        ...
                        &if i >= 14 {
                            format!("BRAM_IOIS_STATUS{}", (i - 14) + j * 4)
                        } else {
                            format!("")
                        },
                    ],
        ...
        self.builder
            .make_int_out_wire("INT.OUT.TBUS", "INT.OUT.TBUS", &["TBUS"]);

        // PCI needs special output wires.
        // XXX register
        self.builder
            .geomdb
            .make_wire("INT.OUT.PCI0", "INT.OUT.PCI", false);
        self.builder
            .geomdb
            .make_wire("INT.OUT.PCI1", "INT.OUT.PCI", false);
    }
    ...
    fn setup_int(&mut self) {
        ...
        if self.is_2vp() {
            ...
            self.builder
                .geomdb
                .make_tile_single("INT_IF_PPC", tslot_int_if);
            self.builder
                .geomdb
                .make_tile_single("INT_IF_GT_0", tslot_int_if);
            self.builder
                .geomdb
                .make_tile_single("INT_IF_GT_123", tslot_int_if);
            self.builder
                .geomdb
                .make_tile_single("INT_IF_GT_4", tslot_int_if);
        }
    }
    ...
    fn setup_buses_clk(&mut self) {
        ...
        for i in 0..8 {
            self.builder.geomdb.make_vbus_wire(
                &format!("CLKBT.GCLK{}", i),
                "CLKBT.GCLK",
                vbus_clkbt,
                false,
        ...
        for i in 0..8 {
            self.builder.geomdb.make_hbus_wire(
                &format!("HROW.GCLK{}", i),
                "HROW.GCLK",
                hbus_hrow,
                false,
    ...
    fn setup_buses_dcm(&mut self) {
        ...
        for i in 0..8 {
            let w = self.builder.geomdb.make_hbus_wire(
                &format!("DCM.CLKPAD{}", i),
                "DCM.CLKPAD",
                hbus_dcm,
                false,
    ...
    fn setup_buses_gt(&mut self) {
        ...
        if self.is_2vp() {
            ...
            self.builder
                .geomdb
                .make_hbus_wire("BREFCLK.BREFCLK", "BREFCLK", hbus_brefclk, false);
            self.builder
                .geomdb
                .make_hbus_wire("BREFCLK.BREFCLK2", "BREFCLK", hbus_brefclk, false);
            let hbus_brefclkx = self.builder.geomdb.make_horiz_bus("BREFCLKX");
            self.builder
                .geomdb
                .make_hbus_wire("BREFCLKX.P", "BREFCLKX", hbus_brefclkx, false);
            self.builder
                .geomdb
                .make_hbus_wire("BREFCLKX.N", "BREFCLKX", hbus_brefclkx, false);
        }
    }
    ...
    fn setup_clk(&mut self) {
        ...
        self.builder
            .geomdb
            .make_tile("CLKC", &[(0, 0, tslot_clk), (0, 1, tslot_clk)]);
        self.builder
            .geomdb
            .make_tile("CLK_B", &[(0, 0, tslot_clk), (1, 0, tslot_clk)]);
        self.builder
            .geomdb
            .make_tile("CLK_T", &[(0, 0, tslot_clk), (1, 0, tslot_clk)]);
        self.builder
            .geomdb
            .make_tile_single("CLK_DCMOUT_B", tslot_clk);
        self.builder
            .geomdb
            .make_tile_single("CLK_DCMOUT_T", tslot_clk);

        // Middle clock distribution tile: from CLKC bus to HROW bus.
        let tslot_hrow = self.builder.geomdb.make_tile_slot("HROW");
        self.builder
            .geomdb
            .make_tile("HROW_M", &[(0, 0, tslot_hrow), (1, 0, tslot_hrow)]);
        if !self.is_2vp() {
            self.builder
                .geomdb
                .make_tile("HROW_B", &[(0, 0, tslot_hrow), (1, 0, tslot_hrow)]);
            self.builder
                .geomdb
                .make_tile("HROW_T", &[(0, 0, tslot_hrow), (1, 0, tslot_hrow)]);
        }
    }
    ...
    fn setup_clkbt(&mut self) {
        for i in 0..8 {
            self.builder
                .geomdb
                .make_wire(&format!("CLKBT.IMUX.SEL{}", i), "CLKBT.IMUX.SEL", false);
        }
        for i in 0..8 {
            self.builder.geomdb.make_wire(
                &format!("CLKBT.IMUX.CLKDUB{}", i),
                "CLKBT.IMUX.CLKDUB",
                false,
            );
        }
        for i in 0..8 {
            self.builder
                .geomdb
                .make_wire(&format!("CLKBT.IMUX.CLK{}", i), "CLKBT.IMUX.CLK", false);
        }
    }
    ...
    fn setup_site_clb(&mut self) {
        ...
        let (pcls_center_e, pcls_center_w) = self.builder.geomdb.make_port_pair(
            ("CENTER_E_PASS", "CENTER_W_PASS"),
            (pslot_center_e, pslot_center_w),
        );
        ...
        // COUT3 -> CIN2 tile up
        for i in 0..4 {
            let co = self.builder.make_tile_wire(
                &format!("CLB.COUT{}", i),
                "CLB.COUT",
                &["SITE_CLB"],
                &format!("COUT{}", i),
            ...
            if i == 1 || i == 3 {
                let co_n = self.builder.make_tile_wire(
                    &format!("CLB.COUT{}.N", i),
                    "CLB.COUT",
                    &["SITE_CLB"],
                    &format!("CIN{}", i - 1),
        ...
        for i in 0..4 {
            let so = self.builder.make_tile_wire(
                &format!("CLB.SOPOUT{}", i),
                "CLB.SOPOUT",
                &["SITE_CLB"],
                &format!("SOPOUT{}", i),
            ...
            if i >= 2 {
                let so_e = self.builder.make_tile_wire(
                    &format!("CLB.SOPOUT{}.E", i),
                    "CLB.SOPOUT",
                    &["SITE_CLB"],
                    &format!("SOPIN{}", i - 2),
                    (0, 0),
                ...
                self.builder
                    .geomdb
                    .make_simple_pconn(so_e, so, pcls_center_e, pcls_center_w);
            }
        }
        ...
        for i in 0..4 {
            let fx = self.builder.make_tile_wire(
                &format!("CLB.FXOUT{}", i),
                "CLB.FXOUT",
                &["SITE_CLB"],
                &format!("FX{}", i),
            ...
            if i == 1 {
                let fx_s = self.builder.make_tile_wire(
                    &format!("CLB.FXOUT{}.S", i),
                    "CLB.FXOUT",
                    &["SITE_CLB"],
                    "FXINA3",
                    (0, 0),
        ...
        // others -> discarded
        for i in 0..4 {
            let dig = self.builder.make_tile_wire(
                &format!("CLB.DIG{}", i),
                "CLB.DIG",
                &["SITE_CLB"],
                &format!("DIG{}", i),
            ...
            if i == 3 {
                let dig_s = self.builder.make_tile_wire(
                    &format!("CLB.DIG{}.S", i),
                    "CLB.DIG",
                    &["SITE_CLB"],
                    "DIG_S3",
        ...
        let (pcls_tbuf_e, pcls_tbuf_w) = self
            .builder
            .geomdb
            .make_port_pair(("TBUF_E_PASS", "TBUF_W_PASS"), (pslot_tbuf_e, pslot_tbuf_w));
        ...
        let tbuf: Vec<_> = (0..4)
            ...
            .collect();
        let tbuf0_w = self.builder.make_tile_wire(
            "CLB.TBUF0.W",
            "CLB.TBUF",
            &["SITE_CLB"],
            "TBUF3_E",
        ...
        self.builder
            .geomdb
            .make_simple_pconn(tbuf0_w, tbuf[0], pcls_tbuf_w, pcls_tbuf_e);
        self.builder
            .geomdb
            .make_simple_pconn(tbuf[0], tbuf[1], pcls_tbuf_w, pcls_tbuf_e);
        self.builder
            .geomdb
            .make_simple_pconn(tbuf[1], tbuf[2], pcls_tbuf_w, pcls_tbuf_e);
        self.builder
            .geomdb
            .make_simple_pconn(tbuf[3], tbuf[2], pcls_tbuf_e, pcls_tbuf_w);
    ...
    fn setup_iob(&mut self) {
        ...
        self.builder.geomdb.make_tile(
            "SITE_IOB_T321010",
            &[(0, 0, tslot_site), (1, 0, tslot_site)],
        );
        self.builder.geomdb.make_tile(
            "SITE_IOB_T323210",
            &[(0, 0, tslot_site), (1, 0, tslot_site)],
        ...
        self.builder
            .geomdb
            .make_tile("SITE_IOB_T210", &[(0, 0, tslot_site)]);
        self.builder
            .geomdb
            .make_tile("SITE_IOB_T321", &[(0, 0, tslot_site)]);
        ...
        self.builder
            .geomdb
            .make_tile("SITE_IOB_B012", &[(0, 0, tslot_site)]);
        self.builder
            .geomdb
            .make_tile("SITE_IOB_B123", &[(0, 0, tslot_site)]);

        self.builder.geomdb.make_tile(
            "SITE_IOB_L321010",
            &[(0, 0, tslot_site), (0, 1, tslot_site)],
        );
        self.builder.geomdb.make_tile(
            "SITE_IOB_L323210",
            &[(0, 0, tslot_site), (0, 1, tslot_site)],
        ...
        let ioi_tiles = &["SITE_IOI", "SITE_IOI_CLK_B", "SITE_IOI_CLK_T"];
        for i in 0..4 {
            self.builder.make_tile_wire(
                &format!("IOI.I{}", i),
                "IOI.I",
    ...
    fn fill_grid_int_bram(&self, part: &mut PartBuilder) {
        ...
        let xy: HashSet<(usize, usize)> = part
            .find_anchors(&TileAnchor::int(&["BRAM0", "BRAM1", "BRAM2", "BRAM3"]))
            .into_iter()
            .map(|(_, xy)| xy)
            .collect();
    ...
    fn fill_grid_bus(&self, part: &mut PartBuilder) {
        ...
        part.fill_srcbrk_bus_split(
            &self.builder,
            Orient::V,
            "HCLK",
            hclk_rows.iter().copied(),
            hbrk_rows.iter().copied(),
        );
    ...
    fn fill_grid_site_conns(&self, part: &mut PartBuilder) {
        ...
        let xy: HashSet<(usize, usize)> = part
            .find_anchors(&TileAnchor::int(&["CENTER"]))
            .into_iter()
            .map(|(_, xy)| xy)
            .collect();
        for (x, y) in xy.iter().copied() {
