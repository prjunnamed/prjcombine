...
use super::cfg::{GeomBuilderConfig, IntTileInfo, IntTermInfo, TieSiteInfo, TileAnchor};
...
impl Series7GeomMaker {
    pub fn new(family: &str) -> Self {
        ...
        let cfg = GeomBuilderConfig {
            ...
            extra_cell_col_injectors: [
                ...
                ("PSS0", &[-28, -27, -21, -20, -17, -16, -11, -10, -7, -6, -1, 0, 3, 4, 9, 10, 13, 14][..]),
            ].iter().copied().collect(),
            int_terms,
            int_bufs: Vec::new(),
            int_dbufs: Vec::new(),
    ...
    fn setup_int_ll(&mut self) {
        ...
        for i in 0..6 {
            ...
            self.builder.connect_int_wire(lh[12-i], Dir::E, lh[12-i-1]);
        }
        ...
        let lvb: Vec<_> = (0..13).map(|i| {
            let w = self.builder.geomdb.make_wire(&format!("INT.LVB{}", i), "INT.LVB", true);
            self.builder.register_int_wire(w, &[
                &format!("LVB{}", i),
                &format!("LVB_L{}", i),
            ]);
        ...
        for i in 0..6 {
            ...
            self.builder.connect_int_wire(lvb[12-i], Dir::S, lvb[12-i-1]);
        }
        self.builder.register_int_wire(lvb[6], &[
            "LVB_L6_SLV",
            "LVB6_SLV",
        ]);
        ...
        let (pcls_int_slv_n, pcls_int_slv_s) = self.builder.geomdb.make_port_pair(("INT_SLV_N", "INT_SLV_S"), (pslot_int_slv_n, pslot_int_slv_s));
        self.pcls_int_slv = Some((pcls_int_slv_n, pcls_int_slv_s));

        self.builder.geomdb.make_simple_pconn(lvb[6], lvb[6], pcls_int_slv_n, pcls_int_slv_s);
    ...
    fn setup_int_hex(&mut self) {
        for (name, da, db, dend) in [
            ...
            for i in 0..4 {
                let beg = self.builder.make_int_wire(&format!("INT.{}BEG{}", name, i), "INT.HEX", &[&format!("{}BEG{}", name, i)]);
                let a = self.builder.make_int_wire(&format!("INT.{}A{}", name, i), "INT.HEX", &[&format!("{}A{}", name, i)]);
                let b = self.builder.make_int_wire(&format!("INT.{}B{}", name, i), "INT.HEX", &[&format!("{}B{}", name, i)]);
                let c = self.builder.make_int_wire(&format!("INT.{}C{}", name, i), "INT.HEX", &[&format!("{}C{}", name, i)]);
                let d = self.builder.make_int_wire(&format!("INT.{}D{}", name, i), "INT.HEX", &[&format!("{}D{}", name, i)]);
                let e = self.builder.make_int_wire(&format!("INT.{}E{}", name, i), "INT.HEX", &[&format!("{}E{}", name, i)]);
                let end = self.builder.make_int_wire(&format!("INT.{}END{}", name, i), "INT.HEX", &[&format!("{}END{}", name, i)]);
                ...
                if i == 0 {
                    if let Some((Dir::S, x)) = dend {
                        let end_s = self.builder.make_int_wire(&format!("INT.{}END_S{}", name, i), "INT.HEX", &[&format!("{}END_S{}_{}", name, x, i)]);
                        self.builder.connect_int_wire(end_s, Dir::S, end);
                    }
                }
                if i == 3 {
                    if let Some((Dir::N, x)) = dend {
                        let end_n = self.builder.make_int_wire(&format!("INT.{}END_N{}", name, i), "INT.HEX", &[&format!("{}END_N{}_{}", name, x, i)]);
    ...
    fn setup_int_quad(&mut self) {
        for (name, dir, dend) in [
            ("EE4", Dir::E, None),
            ("WW4", Dir::W, Some((Dir::S, 0))),
        ].iter().copied() {
            for i in 0..4 {
                let beg = self.builder.make_int_wire(&format!("INT.{}BEG{}", name, i), "INT.QUAD", &[&format!("{}BEG{}", name, i)]);
                let a = self.builder.make_int_wire(&format!("INT.{}A{}", name, i), "INT.QUAD", &[&format!("{}A{}", name, i)]);
                let b = self.builder.make_int_wire(&format!("INT.{}B{}", name, i), "INT.QUAD", &[&format!("{}B{}", name, i)]);
                let c = self.builder.make_int_wire(&format!("INT.{}C{}", name, i), "INT.QUAD", &[&format!("{}C{}", name, i)]);
                let end = self.builder.make_int_wire(&format!("INT.{}END{}", name, i), "INT.QUAD", &[&format!("{}END{}", name, i)]);
                ...
                if i == 0 {
                    if let Some((Dir::S, x)) = dend {
                        let end_s = self.builder.make_int_wire(&format!("INT.{}END_S{}", name, i), "INT.QUAD", &[&format!("{}END_S{}_{}", name, x, i)]);
                        self.builder.connect_int_wire(end_s, Dir::S, end);
                    }
                }
            }
    ...
    fn setup_int_dbl(&mut self) {
        for (name, da, db, dend) in [
            ...
            for i in 0..4 {
                ...
                let a = self.builder.make_int_wire(&format!("INT.{}A{}", name, i), "INT.DBL", &[&format!("{}A{}", name, i)]);
                ...
                if i == 0 {
                    if let Some((Dir::S, x)) = dend {
                        let end_s = self.builder.make_int_wire(&format!("INT.{}END_S{}", name, i), "INT.DBL", &[&format!("{}END_S{}_{}", name, x, i)]);
                        self.builder.connect_int_wire(end_s, Dir::S, end);
                    }
                }
                if i == 3 {
                    if let Some((Dir::N, x)) = dend {
                        let end_n = self.builder.make_int_wire(&format!("INT.{}END_N{}", name, i), "INT.DBL", &[&format!("{}END_N{}_{}", name, x, i)]);
                        self.builder.connect_int_wire(end_n, Dir::N, end);
                    }
                }
            }
        ...
        for (name, dir, dbeg, dend) in [
            ...
            for i in 0..4 {
                if dir == Dir::N && dbeg == Some(Dir::N) && i == 3 {
                    self.builder.make_int_wire(&format!("INT.SNG.{}BEG_N{}", name, i), "INT.SNG", &[&format!("{}BEG_N{}", name, i)]);
                } else if dir == Dir::S && dbeg == Some(Dir::S) && i == 0 {
                    self.builder.make_int_wire(&format!("INT.SNG.{}BEG_S{}", name, i), "INT.SNG", &[&format!("{}BEG_S{}", name, i)]);
                } else {
                    let beg = self.builder.make_int_wire(&format!("INT.SNG.{}BEG{}", name, i), "INT.SNG", &[&format!("{}BEG{}", name, i)]);
                    let end = self.builder.make_int_wire(&format!("INT.SNG.{}END{}", name, i), "INT.SNG", &[&format!("{}END{}", name, i)]);
                    self.builder.connect_int_wire(end, dir, beg);

                    if i == 0 {
                        if dbeg == Some(Dir::S) {
                            let beg_s = self.builder.make_int_wire(&format!("INT.SNG.{}BEG_S{}", name, i), "INT.SNG", &[&format!("{}BEG_S{}", name, i)]);
                            self.builder.connect_int_wire(beg, Dir::N, beg_s);
                        }
                        if let Some((Dir::S, x)) = dend {
                            let end_s = self.builder.make_int_wire(&format!("INT.SNG.{}END_S{}", name, i), "INT.SNG", &[&format!("{}END_S{}_{}", name, x, i)]);
                            self.builder.connect_int_wire(end_s, Dir::S, end);
                        }
                    }
                    if i == 3 {
                        if dbeg == Some(Dir::N) {
                            let beg_n = self.builder.make_int_wire(&format!("INT.SNG.{}BEG_N{}", name, i), "INT.SNG", &[&format!("{}BEG_N{}", name, i)]);
                            self.builder.connect_int_wire(beg, Dir::S, beg_n);
                        }
                        if let Some((Dir::N, x)) = dend {
                            let end_n = self.builder.make_int_wire(&format!("INT.SNG.{}END_N{}", name, i), "INT.SNG", &[&format!("{}END_N{}_{}", name, x, i)]);
                            self.builder.connect_int_wire(end_n, Dir::N, end);
                        }
                    }
                }
    ...
    fn setup_int_imux(&mut self) {
        ...
        for i in 0..2 {
            self.builder.make_int_wire(&format!("INT.IMUX.CTRL{}", i), "INT.IMUX.CTRL", &[
                &format!("CTRL{}", i),
                &format!("CTRL_L{}", i),
            ]);
        }
        // XXX
        for i in 0..8 {
            self.builder.make_int_wire(&format!("INT.IMUX.BYP{}", i), "INT.IMUX.BYP", &[&format!("BYP_ALT{}", i)]);
            self.builder.make_int_wire(&format!("INT.IMUX.BYP{}.SITE", i), "INT.IMUX.BYP_SITE", &[
                &format!("BYP{}", i),
                &format!("BYP_L{}", i),
            ]);
            let w = self.builder.make_int_wire(&format!("INT.IMUX.BYP{}.BOUNCE", i), "INT.IMUX.BYP_BOUNCE", &[&format!("BYP_BOUNCE{}", i)]);
            if i == 2 || i == 3 || i == 6 || i == 7 {
                let w_n = self.builder.make_int_wire(&format!("INT.IMUX.BYP{}.BOUNCE.N", i), "INT.IMUX.BYP_BOUNCE", &[&format!("BYP_BOUNCE_N3_{}", i)]);
                self.builder.connect_int_wire(w_n, Dir::N, w);
            }
        }
        for i in 0..8 {
            self.builder.make_int_wire(&format!("INT.IMUX.FAN{}", i), "INT.IMUX.FAN", &[&format!("FAN_ALT{}", i)]);
            self.builder.make_int_wire(&format!("INT.IMUX.FAN{}.SITE", i), "INT.IMUX.FAN_SITE", &[
                &format!("FAN{}", i),
                &format!("FAN_L{}", i),
            ]);
            let w = self.builder.make_int_wire(&format!("INT.IMUX.FAN{}.BOUNCE", i), "INT.IMUX.FAN_BOUNCE", &[&format!("FAN_BOUNCE{}", i)]);
            if i == 0 || i == 2 || i == 4 || i == 6 {
                let w_s = self.builder.make_int_wire(&format!("INT.IMUX.FAN{}.BOUNCE.S", i), "INT.IMUX.FAN_BOUNCE", &[&format!("FAN_BOUNCE_S3_{}", i)]);
                self.builder.connect_int_wire(w_s, Dir::S, w);
            }
        }
        for i in 0..48 {
            self.builder.make_int_wire(&format!("INT.IMUX.IMUX{}", i), "INT.IMUX.IMUX", &[
                &format!("IMUX{}", i),
                &format!("IMUX_L{}", i),
            ]);
        }
    ...
    fn setup_int_out(&mut self) {
        for i in 0..24 {
            self.builder.make_int_out_wire(&format!("INT.OUT{}", i), "INT.OUT", &[
                &format!("LOGIC_OUTS{}", i),
    ...
    fn setup_int_clk(&mut self) {
        ...
        let (pcls_int_clk_e, pcls_int_clk_w) = self.builder.geomdb.make_port_pair(("INT_CLK_E", "INT_CLK_W"), (pslot_int_clk_e, pslot_int_clk_w));
        self.pcls_int_clk = Some((pcls_int_clk_e, pcls_int_clk_w));

        for i in 0..6 {
            let w = self.builder.make_int_wire(&format!("HCLK.CLK{}.INT", i), "HCLK.CLK.INT", &[
                &format!("GCLK_L_B{}", i),
                &format!("GCLK_B{}_EAST", i),
            ]);
            let w2 = self.builder.make_int_wire(&format!("HCLK.CLK{}.INT.OTHER", i), "HCLK.CLK.INT", &[
                &format!("GCLK_B{}_WEST", i),
            ]);
            self.builder.geomdb.make_simple_pconn(w, w2, pcls_int_clk_w, pcls_int_clk_e);
        }
        for i in 6..12 {
            let w = self.builder.make_int_wire(&format!("HCLK.CLK{}.INT", i), "HCLK.CLK.INT", &[
                &format!("GCLK_L_B{}_WEST", i),
                &format!("GCLK_B{}", i),
            ]);
            let w2 = self.builder.make_int_wire(&format!("HCLK.CLK{}.INT.OTHER", i), "HCLK.CLK.INT", &[
                &format!("GCLK_B{}_EAST", i),
            ]);
            self.builder.geomdb.make_simple_pconn(w, w2, pcls_int_clk_e, pcls_int_clk_w);
        }
    ...
    fn setup_buses_clk(&mut self) {
        ...
        for i in 0..6 {
            let w = self.builder.geomdb.make_vbus_wire(&format!("HCLK.CLK{}", i), "HCLK.CLK", vbus_hclk, false);
            self.builder.register_int_wire(w, &[
                &format!("GCLK_B{}", i),
            ]);
        }
    ...
    fn curse_tiles(&mut self, grid: &mut PartBuilder) {
        for x in 0..grid.rd.width {
            ...
            let crd_b = rawdump::Coord{x, y: grid.rd.height-2};
    ...
    fn fill_grid_int_clk(&mut self, part: &mut PartBuilder) {
        ...
        let xy: Vec<(usize, usize)> = part.find_anchors(&TileAnchor::int(&["INT_L", "INT_L_SLV", "INT_L_SLV_FLY"])).into_iter().map(|(_, xy)| xy).collect();
        for (x, y) in xy {
            let ox = x + 1;
            part.grid.fill_port_pair(&self.builder.geomdb, (x, y), (ox, y), self.pcls_int_clk.unwrap());
        }
    }

    fn fill_grid_int_slv(&mut self, part: &mut PartBuilder) {
        // INT SLV connections special.
        let xy: HashSet<(usize, usize)> = part.find_anchors(&TileAnchor::int(&["INT_L_SLV_FLY", "INT_R_SLV_FLY"])).into_iter().map(|(_, xy)| xy).collect();
        for (x, y) in xy.iter().copied() {
            ...
            assert_eq!(ys % 50, 0);
            for i in 0..49 {
                part.grid.fill_port_pair(&self.builder.geomdb, (x, ys+i), (x, yn+i), self.pcls_int_slv.unwrap());
            }
        }
    }
